...
!-----------------------------------------------------------------------
    ...
    ! column water state variable - co2dp   ! Mars

    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='CO2DP', xtype=ncd_double,  &
            dim1name='column', &
            long_name='CO2 Frost Mass', units='kg/m2')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='CO2DP', data=cps%co2dp, &
            dim1name=namec, &
            ncid=ncid, flag=flag, readvar=readvar)
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
    ...
    ! column water state variable - fracco2   ! Mars

    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='FRAC_CO2', xtype=ncd_double,  &
            dim1name='column', &
            long_name='CO2 Frost Fraction', units='fraction')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='FRAC_CO2', data=cps%frac_co2, &
            dim1name=namec, &
            ncid=ncid, flag=flag, readvar=readvar)
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
