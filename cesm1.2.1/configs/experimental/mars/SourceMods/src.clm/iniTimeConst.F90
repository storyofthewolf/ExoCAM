...
!
  ...
  real(r8), pointer :: ithincol_i(:)      ! soil thermal inertia factor for soil heating  ! mars
...
!EOP
  ...
  integer ,pointer :: soir2d(:)      ! read in - soil roughness length   ! mars
  ...
  real(r8),pointer :: thin2d(:)    ! read in thermal interia !mars         
...
!------------------------------------------------------------------------
  ...
  allocate(thin2d(begg:endg))  ! mars, allocate thermal intertia input
  ...
  !! added for Mars
  call ncd_io(ncid=ncid, varname='THM_INERTIA', flag='read', data=thin2d, dim1name=grlnd, readvar=readvar)
...
! Kludge Wolf Jan 2021, what the hell is this sqrt(pi/cday) factor?
      ithincol_i(c) = 250.0*sqrt(SHR_CONST_PI/SHR_CONST_CDAY)

      ! Soil thermal conductivity from inertia
      !factor in front of thin2d is to test effect of changing thermal inertia
 !     ithincol(c) = ((SHR_CONST_THINFAC*thin2d(g))**2)/(SHR_CONST_SOILBD*SHR_CONST_SOILCP)  !mars
     ithincol(c) = ((SHR_CONST_THINFAC*250.0)**2)/(SHR_CONST_SOILBD*SHR_CONST_SOILCP)  !mars
      ...
      if (ltype(l)==istdlak .or. ltype(l)==istwet .or. ltype(l)==istice .or. ltype(l)==istice_mec) then
         ...
         do lev = 1,nlevgrnd
            ...
            tkdry(c,lev)  = ((0.135_r8*bd + 64.7_r8) / (2.7e3_r8 - 0.947_r8*bd))*(1._r8-om_frac) + &
                            om_tkd*om_frac  
! this was a mismatch between the "sand" and mars soils
!            csol(c,lev)   = ((1._r8-om_frac)*(2.128_r8*sand+2.385_r8*clay) / (sand+clay) +   &
!                           om_csol*om_frac)*1.e6_r8  ! J/(m3 K)  
            csol(c,lev) = SHR_CONST_SOILBD * SHR_CONST_SOILCP  ! mars
