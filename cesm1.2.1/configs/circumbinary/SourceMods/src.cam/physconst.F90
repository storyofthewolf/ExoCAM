module physconst
   ...
   use shr_const_mod, only: shr_const_g,      shr_const_stebol, shr_const_tkfrz,  &
                            ...
                            shr_const_tktrip, shr_const_tkfrz,  shr_const_scon
   ...
   ! Molecular weights
   real(r8), public, parameter :: mwn2        =  28._r8             ! molecular weight n2
   real(r8), public, parameter :: mwh2        =  2._r8              ! molecular weight h2
   real(r8), public, parameter :: mwar        =  40._r8             ! molecular weight ar
   ...
   ! modifiable physical constants for aquaplanet (from physconst_nl, not currently used for exoplanet mod)
   ...
   !   
      ...
      if (masterproc) then
        ...
        if (do_exo_circumbinary) then
          write(iulog,*) 'SOLAR SPECTRUM FILE: ', exo_solar_file_2
          write(iulog,*) 'CIRCUMBINARY TEFF ', cb_teff
          write(iulog,*) 'CIRCUMBINARY STELLAR MASS ', cb_mass
          write(iulog,*) 'CIRCUMBINARY PLANET MASS ', planet_mass
          write(iulog,*) 'CIRCUMBINARY RADII ', cb_radius
          write(iulog,*) 'CIRCUMBINARY LUMINOSITY ', cb_lum
          write(iulog,*) 'CIRCUMBINARY SPIN ', cb_spin
          write(iulog,*) 'CIRCUMBINARY BINARY SEPARATION ', cb_separation
        end if
        write(iulog,*) '******************************************************'
