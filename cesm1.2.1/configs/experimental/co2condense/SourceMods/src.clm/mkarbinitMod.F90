...
!
! !USES:
    use shr_const_mod, only : SHR_CONST_TKFRZ, SHR_CONST_SOILW,SHR_CONST_SOILWN,SHR_CONST_SOILWS, & ! mars
                              SHR_CONST_PI   ! mars
    use clmtype
...
!
    ...
    real(r8), pointer :: t_ref1m(:)         ! 1 m height surface air temperature (Kelvin)  ! Mars
    ...
    real(r8), pointer :: co2dp(:)           ! co2 frost column mass (kg/m**2)  ! mars
    real(r8), pointer :: co2_mass_change(:) ! co2 frost column mass change (kg/m**2) ! mars
    real(r8), pointer :: co2_temp_change(:) ! co2 driven temperature change (K) ! mars
    ...
    real(r8), pointer :: latdeg(:)             ! latitude (radians)   ! mars
    integer,  pointer :: pgridcell(:)      ! gridcell associated with each pft  ! mars
!
!
...
!-----------------------------------------------------------------------
    ...
    co2_mass_change  => cps%co2_mass_change  ! mars
    co2_temp_change  => ces%co2_temp_change  ! mars
    ...
    pgridcell => pft%gridcell ! mars
    latdeg    => grc%latdeg ! mars
    ...
    do c = begc,endc
       ...
       ! CO2 Frost Depth         ! mars
       co2dp(c) = 0.             ! mars
       ! CO2 mass change         ! mars
       co2_mass_change(c) = 0.   ! mars
       ! CO2 temp change         ! mars
       co2_temp_change(c) = 0.   ! mars
    ...
    do c = begc,endc
       ...
       if (.not. lakpoi(l)) then  !not lake
          ...
          if (ltype(l)==istice .or. ltype(l)==istice_mec) then
             ...
             do j = 1, nlevgrnd
                t_soisno(c,j) = 274._r8
                !t_soisno(c,j) = 220._r8  ! mars
             end do
          endif
          t_grnd(c) = t_soisno(c,snl(c)+1)
       else                     !lak
          t_lake(c,1:nlevlak) = 220._r8  ! frozen lake start
          !t_lake(c,1:nlevlak) = 277._r8 ! liquid lake start
          t_grnd(c) = t_lake(c,1)
       endif
    ...
    do p = begp, endp
       ...
       if (use_vancouver) then
          ...
          t_ref2m(p) = 283._r8
          t_ref1m(p) = 283._r8
          !t_veg(p) = 220._r8  ! mars
          !t_ref2m(p) = 220._r8  ! mars
          !t_ref1m(p) = 220._r8  ! mars
          if (urbpoi(l)) then
             t_ref2m_u(p) = 283._r8
          else
             t_ref2m_u(p) = spval
...
!                  h2osoi_vol(c,j) = SHR_CONST_SOILW !  was 0.3_r8 ! mars
                  lat = latdeg(pgridcell(p)) * SHR_CONST_PI / 180._r8
                  if (lat .ge. -20.) then   ! mars
                     h2osoi_vol(c,j) = SHR_CONST_SOILWN   ! mars
                  else   
                     h2osoi_vol(c,j) = SHR_CONST_SOILWS
                  endif  !/mars
                   h2osoi_vol(c,j) = 0.3_r8
                else
                   h2osoi_vol(c,j) = 0.0_r8
