...
!WOLF       if ((coszen(c_idx) > 0._r8) .and. (h2osno_lcl > min_snw)) then     
       if (h2osno_lcl > min_snw) then     
          ...
          do bnd_idx = 1,numrad_snw
             ...
             mu_not    = max(0.001,coszen(c_idx))  ! must set here, because of error handling
...
!WOLF       elseif ( (coszen(c_idx) > 0._r8) .and. (h2osno_lcl < min_snw) .and. (h2osno_lcl > 0._r8) ) then
