...
! Take mass from west (essentially adjusting fx(i,j))
    ...
    do j=j1,j2
       ...
       if ( qmin < D0_0 ) then
!  ETW_AC_PATCH_2026_08_21
!  A negative survived both sweeps, which means no donor existed anywhere on
!  this latitude row -- the all-zero-row case that arises when a tracer is
!  injected into a single column of an otherwise empty field. The deficit is
!  unpayable by construction, so shuffling it further only corrupts the column
!  mass and, through te_map, every hybrid level. Clamp it here instead. The
!  mass so created is bounded by the residual itself, which on a zero row is
!  the PPM overshoot of a zero field (~1e-200), i.e. nothing. Reported
!  distinctly from a genuine partial-fill failure so the two are separable in
!  the log.
          write(iulog,*) ' filew unfillable (zero-donor row), clamped: i, j, qtmp, q = ', &
               imin, jmin, qtmp(jmin,imin), q(imin,jmin)
          do i=1,im
             if (qtmp(j,i) < D0_0) qtmp(j,i) = D0_0
          enddo
       end if
