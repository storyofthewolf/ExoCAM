    ...
    elseif(rk_clouds) then
       ...
       do k=1,pver
          do i = 1,ncol
             ...
             ! Wolf.  If cloud fraction is small, then set cloud amount to  zero.
 
             if (cld(i,k) >= 0.01) then
               cicewp(i,k) = gicewp(i,k) / cld(i,k)               ! In-cloud ice water path.
               cliqwp(i,k) = gliqwp(i,k) / cld(i,k)               ! In-cloud liquid water path.
             else 
                cicewp(i,k) = 0.0               ! In-cloud ice water path.
                cliqwp(i,k) = 0.0               ! In-cloud liquid water path.
             endif
!
