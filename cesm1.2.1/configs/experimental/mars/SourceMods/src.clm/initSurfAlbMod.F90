...
! !USES:
    ...
    use shr_const_mod       , only : SHR_CONST_PI, SHR_CONST_DENSITYCO2FR
    use clmtype
    ...
    use clm_time_manager    , only : get_step_size, get_curr_calday_rotation
...
!
    ...
    real(r8), pointer :: co2dp(:)           ! co2 frost mass (kg/m2) ! mars
...
!    real(r8), pointer :: co2_mass_change(:) ! co2 frost mass (kg/m2) ! mars
    real(r8), pointer :: frac_co2(:)       ! fraction of ground covered by co2 frost (0 to 1) 
...
!$OMP PARALLEL DO PRIVATE (nc,p,j,l,c,fc,begg,endg,begl,endl,begc,endc,begp,endp,lat,temp,snowbd,fmelt)
    do nc = 1,nclumps
       ...
       do c = begc, endc  ! mars
        frac_co2(c) = (co2dp(c)/SHR_CONST_DENSITYCO2FR)/(10.* z0m_ini(c) + snowdp(c) + co2dp(c)/SHR_CONST_DENSITYCO2FR)
       end do  !mars 
