...
contains
  ...
  subroutine carma_register
    ...
    real(r8)          :: rrat(NBIN)               ! ratio of max radius to equivalent sphere
    real(r8)          :: df(NBIN)                 ! fractal_dimension
    real(r8)          :: nmon(NBIN)               ! number of monomers
      ...
      if (masterproc) then
        ...
        ! Is this an initial or restart run?
          ...
          if (do_print) write(LUNOPRT,*) 'Creating CARMA reference temperature file ... ', trim(locfn)
            write(LUNOPRT,*) "pref_mid", pref_mid(:) / 100._r8
            write(LUNOPRT,*) "carma_t_ref",  carma_t_ref(:)
