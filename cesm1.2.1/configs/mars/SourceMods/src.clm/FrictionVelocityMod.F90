...
! !INTERFACE:
  subroutine FrictionVelocity(lbn, ubn, fn, filtern, &
                              ...
                              temp1, temp2, temp12m, temp22m, temp11m, fm, landunit_index)
!
! !DESCRIPTION:
! Calculation of the friction velocity, relation for potential
! temperature and humidity profiles of surface boundary layer.
...
! !ARGUMENTS:
   ...
   real(r8), intent(out) :: temp11m(lbn:ubn) ! relation for specific humidity profile applied at 2-m  ! mars
...
! If forcing height is less than or equal to 10m, then set 10-m wind to um
      ...
      ! Temperature profile applied at 1-m ! Mars

      zldis(n) = 1.0_r8 + z0h(n)
      zeta(n) = zldis(n)/obu(n)
      if (zeta(n) < -zetat) then
         temp11m(n) = vkc/(log(-zetat*obu(n)/z0h(n))&
              - StabilityFunc2(-zetat) &
              + StabilityFunc2(z0h(n)/obu(n)) &
              + 0.8_r8*((zetat)**(-0.333_r8)-(-zeta(n))**(-0.333_r8)))
      else if (zeta(n) < 0._r8) then
         temp11m(n) = vkc/(log(zldis(n)/z0h(n)) &
              - StabilityFunc2(zeta(n))  &
              + StabilityFunc2(z0h(n)/obu(n)))
      else if (zeta(n) <=  1._r8) then
         temp11m(n) = vkc/(log(zldis(n)/z0h(n)) + 5._r8*zeta(n) - 5._r8*z0h(n)/obu(n))
      else
         temp11m(n) = vkc/(log(obu(n)/z0h(n)) + 5._r8 - 5._r8*z0h(n)/obu(n) &
              + (5._r8*log(zeta(n))+zeta(n)-1._r8))
      end if  ! /Mars
