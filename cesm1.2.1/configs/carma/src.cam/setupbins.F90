...
!           endif
  ...
  do igrp = 1, NGROUP
    ...
    if (do_print_init) write(LUNOPRT,'(/,a)') 'CARMA bins' 
    if (do_print_init) write(LUNOPRT,'(/,a)') 'igrp, j, r(j,igrp), nmon(j,igrp), rf'
    do j = 1, NBIN
      ...
      ! fractal flag is true
        ...
        elseif (r(j,igrp) .gt. rmon(igrp)) then   ! if bin radius is greater than the monomer size
      ...
      if (do_print_init) write(LUNOPRT,7) igrp, j, r(j,igrp), nmon(j,igrp), df(j,igrp), rf
   enddo
  enddo
