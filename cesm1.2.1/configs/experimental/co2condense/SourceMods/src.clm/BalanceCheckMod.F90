...
!
    ...
    real(r8), pointer :: co2ice_lh(:)          ! co2 ice latent heat    ! mars
...
!
    ...
    real(r8), pointer :: co2ice_lh(:)          ! CO2 latent heat release/uptake    ! Mars
...
!
    ...
    real(r8), pointer :: errsoi_pft(:)      ! column-level soil/lake energy conservation error (W/m**2)  ! mars, why did I add this?
...
!-----------------------------------------------------------------------
    ...
    do p = lbp, ubp
       ...
       if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
          ...
          !
          ! The energetic effect of latent heat exhcange of CO2 is implicitly included in the 
          ! energy balance by direct modification of the surface temperature, which updates eflx_lwrad_out
          if (ltype(l) /= isturb) then
    ...
    do p = lbp, ubp
       ...
       if ( found  .and. (nstep > 2) ) then
          ...
          write(iulog,*)' co2ice_lh      = ',co2ice_lh(indexp)  
          write(iulog,*)'clm model is stopping'
!          call endrun()
      end if
    end do
    ...
    do c = lbc, ubc
       ...
       if ( found ) then
         write(iulog,100)'BalanceCheck: soil balance error, col level',nstep,indexc,errsoi_col(indexc)
         if (abs(errsoi_col(indexc)) > .10_r8 .and. (nstep > 2) ) then
            write(iulog,*)' eflx_lwrad_net = ',eflx_lwrad_net(indexc)
            write(iulog,*)' eflx_lh_tot    = ',eflx_lh_tot(indexc)
            write(iulog,*)' eflx_soil_grnd = ',eflx_soil_grnd(indexc)
            write(iulog,*)' co2ice_lh      = ',co2ice_lh(indexc)
            write(iulog,*)'clm model is stopping'
 !           call endrun()                                                                                              
         end if
      end if
