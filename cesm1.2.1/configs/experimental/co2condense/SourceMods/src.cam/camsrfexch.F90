...
! ETW Mars kludge
     real(r8) :: co2srf_snow(pcols)  ! CO2 snow fall flux [kg/m2/s]
     real(r8) :: co2srf_cond(pcols)  ! CO2 surface condensation flux [kg/m3/s]
...
!---------------------------------------------------------------------------
  ...
  type cam_in_t    
     ...
     real(r8) :: srf_emiss(pcols)        ! Surface emissivity ! mars
     real(r8) :: co2dp(pcols)            ! co2 frost mass ! mars
     real(r8) :: co2_mass_change(pcols)  ! co2 frost mass change ! mars
     real(r8) :: co2_lhflx(pcols)        ! co2 latent heat flux
...
! need to add
! specific heat of the surface grid box
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_in(c)%srf_emiss       (:) = 1._r8  ! mars
       cam_in(c)%co2dp           (:) = 0._r8  ! mars
       cam_in(c)%co2_mass_change (:) = 0._r8  ! mars
       cam_in(c)%co2_lhflx       (:) = 0._r8  ! mars
       if ( dust ) then
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_out(c)%co2srf_snow(:)  = 0._r8 ! CO2 snow fall flux to surface [kg/m2/s] 
       cam_out(c)%co2srf_cond(:)  = 0._r8 ! CO2 ice condensation mass flux to surface [kg/m3/s] 
    end do
