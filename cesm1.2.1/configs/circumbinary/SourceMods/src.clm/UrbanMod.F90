...
! !USES:
  ...
  use clm_varcon  , only : isecspday, degpsec, zvir
...
!-----------------------------------------------------------------------
    ...
    do ib = 1,numrad
       ...
       do fp = 1,num_urbanp
          ...
          fabd(p,ib)    = 0._r8
          fabi(p,ib)    = 0._r8
!Wolf          if (coszen_pft(fp) > 0._r8) then
             ftdd(p,ib) = 1._r8
!Wolf          else
!Wolf             ftdd(p,ib) = 0._r8
!Wolf          end if
          ftid(p,ib)    = 0._r8
!Wolf          if (coszen_pft(fp) > 0._r8) then
             ftii(p,ib) = 1._r8
!Wolf          else
!Wolf             ftii(p,ib) = 0._r8
!Wolf          end if
          omega(p,ib)   = 0._r8
          if (ib == 1) then
             gdir(p)    = 0._r8
...
!-----------------------------------------------------------------------
    ...
    do fl = 1,num_urbanl
       l = filter_urbanl(fl)
       do c = coli(l),colf(l)
!Wolf          if (coszen(fl) > 0._r8 .and. h2osno(c) > 0._r8) then
             if (ctype(c) == icol_roof) then
...
! Compute fluxes - Follows CLM approach for bare soils (Oleson et al 2004)
    ...
    do fl = 1, num_urbanl
       ...
       thv_g(l) = forc_th(g)*(1._r8+zvir*forc_q(g))
       dth(l)   = thm_g(l)-taf(l)
       dqh(l)   = forc_q(g)-qaf(l)
       dthv     = dth(l)*(1._r8+zvir*forc_q(g))+zvir*forc_th(g)*dqh(l)
    ...
    do iter = 1,niters
     ...
     ! temperature and humidity profiles of surface boundary layer.
      ...
      do fl = 1, num_urbanl
         ...
         tstar = temp1(l)*dth(l)
         qstar = temp2(l)*dqh(l)
         thvstar = tstar*(1._r8+zvir*forc_q(g)) + zvir*forc_th(g)*qstar
