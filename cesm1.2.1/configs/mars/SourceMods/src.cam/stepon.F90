...
!-----------------------------------------------------------------------
   ...
   !-----------------------------------------
   do k = 1, km+1
!      ak(k) = hyai(k) * D1E5             ! mars-wolf
      ak(k) = hyai(k) * SHR_CONST_PSTD  ! mars-wolf
      bk(k) = hybi(k)
      if( bk(k) == D0_0 ) ks = k-1
   end do
   ptop = ak(1)
   if ( iam == 0 ) then
     ...
     write(iulog,*) 'level, pressure'
      do k=1, km+1
       write(iulog,*) k, ak(k), bk(k)
      end do
     write(iulog,*) '-----------------------------------------------------'
    endif
...
!
! !INTERFACE:
subroutine stepon_run2( phys_state, phys_tend, dyn_in, dyn_out, cam_in ) ! mars
! !USES:
   use dp_coupling,      only: p_d_coupling
   use camsrfexch,          only: cam_in_t ! mars
!
! !INPUT/OUTPUT PARAMETERS:
!
   type(physics_state), intent(inout):: phys_state(begchunk:endchunk)
   type(physics_tend), intent(inout):: phys_tend(begchunk:endchunk)
   type(cam_in_t), intent(inout), dimension(begchunk:endchunk) :: cam_in ! mars
