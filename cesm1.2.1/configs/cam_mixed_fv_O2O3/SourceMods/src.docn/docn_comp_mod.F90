...
!EOP
    ...
    logical       :: force_restart  ! force use temperature fields from restart file (docn.r) from namelist
    logical       :: exists      ! file existance
    integer(IN)   :: nu          ! unit number

    !----- define namelist -----
    namelist / docn_nml / &
        ocn_in, decomp, restfilm, restfils, force_restart
...
!-------------------------------------------------------------------------------
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    if (my_task == master_task) then
       ...
       write(logunit,*)'(docn_comp_init) force_restart = ',force_restart
    endif
    call shr_mpi_bcast(ocn_in,mpicom,'ocn_in')
    call shr_mpi_bcast(decomp,mpicom,'decomp')
    call shr_mpi_bcast(restfilm,mpicom,'restfilm')
    call shr_mpi_bcast(restfils,mpicom,'restfils')
    call shr_mpi_bcast(force_restart,mpicom,'force_restart')

    if (force_restart) read_restart = .true. 
