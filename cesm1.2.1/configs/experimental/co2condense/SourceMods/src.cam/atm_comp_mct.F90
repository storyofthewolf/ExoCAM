module atm_comp_mct
  ...
  use exo_radiation_cam_intr, only: exo_radiation_nextsw_cday
...
!       nextsw_cday = radiation_nextsw_cday() 
       ...
       if (nstep < 1 .or. dtime < atm_cpl_dt) then
          nextsw_cday = exo_radiation_nextsw_cday() 
  ...
  subroutine atm_import_mct( x2a_a, cam_in )
    ...
    do c=begchunk,endchunk
       ...
       do i =1,ncols                                                               
          ...
          cam_in(c)%srf_emiss(i)       =  x2a_a%rAttr(index_x2a_Sl_srf_emiss, ig)        ! mars
          cam_in(c)%co2dp(i)           =  x2a_a%rAttr(index_x2a_Sl_co2dp, ig)            ! mars
          cam_in(c)%co2_mass_change(i) =  x2a_a%rAttr(index_x2a_Sl_co2_mass_change, ig)  ! mars
          cam_in(c)%co2_lhflx(i)       = -x2a_a%rAttr(index_x2a_Faxx_lat_co2, ig)        ! mars
  ...
  subroutine atm_export_mct( cam_out, a2x_a )
    ...
    do c=begchunk, endchunk
	  ...
	  a2x_a%rAttr(index_a2x_Sa_dens   ,ig) = cam_out(c)%rho(i)
          ...
          ! CO2 condensation
          a2x_a%rAttr(index_a2x_Faxa_co2srf_snow,ig) = cam_out(c)%co2srf_snow(i)  ! snow fall flux [kg/m2/s]
          a2x_a%rAttr(index_a2x_Faxa_co2srf_cond,ig) = cam_out(c)%co2srf_cond(i)  ! condensation flux [kg/m3/s]
