...
! !USES:
    ...
    use shr_const_mod      , only : SHR_CONST_PI, SHR_CONST_CBL, SHR_CONST_DLAPSE, & ! mars
                                    SHR_CONST_TKFRZ, SHR_CONST_DENSITYCO2FR, SHR_CONST_CO2_EMIS
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(r8), pointer :: z0m_ini(:)  ! roughness length mars
    real(r8), pointer :: co2dp(:)    ! CO2 surface ice mass (kg/m2)  ! Mars
    real(r8), pointer :: frac_co2(:) ! Mars
...
!
    ...
    real(r8), pointer :: srf_emiss(:)     !surface emissivity passed to atmosphere model ! MARS ETW
...
!    forc_th       => clm_a2l%forc_th   ! mars
    ...
    do fc = 1,num_nolakec
       ...
       ! Mars NOTE: There seems to be a problem when there is very little to no water available.
       ! Trying to get around this by setting hr to 0 leads to a latent heat conservation issue.
       ! Maybe this is a problem coming from there being so little water on Mars? (Urata 10/27/09)
       ...
       ! Lake emissivities are set elsewhere
       if (ityplun(l) /= isturb) then
         ! Wolf simplified logic: set ground emissivity to 1 everywhere.
         ! If the CO2 fraction dominates, then set SHR_CONST_CO2_EMIS 
         ! This may need to be revisited when I introduce water to the system
         emg(c) = 1.0_r8  ! set ground emissivity to 1 for all surfaces
         if (frac_co2(c) > frac_sno(c)) emg(c) = SHR_CONST_CO2_EMIS !
         srf_emiss(c) = emg(c)  ! set surface emissivity to be passed to atmosphere model

         ! prior logic can be used to set emissivity for diffent ltypes
         !if (ityplun(l)==istice .or. ityplun(l)==istice_mec) then
         !  !emg(c) = 0.97_r8
         !  emg(c) = 1.0_r8  ! Wolf set ground emissivity to 1 for ice
         !else   ! not ice, compute emissivity by snow fraction
         !  !emg(c) = (1._r8-frac_sno(c))*0.96_r8 + frac_sno(c)*0.97_r8
         !  emg(c) = (1._r8-frac_sno(c))*1.0_r8 + frac_sno(c)*1.0_r8
         !end if
       end if
       !write(*,*) "frac_co2, frac_sno, emg", frac_co2(c),frac_sno(c), emg(c), srf_emiss(c)
       ...
       if (frac_sno(c) > 0._r8) then
          ...
          !z0mg(c) = zlnd        ! mars, this use a constant roughless length
          z0mg(c) = z0m_ini(c)  ! mars, uncomment to use read in roughness length
       end if
       z0hg(c) = z0mg(c)            ! initial set only
       z0qg(c) = z0mg(c)            ! initial set only

!if(masterproc) write(*,*) "biogeophysics1mod, z0", z0mg(c), z0m_ini(c)
...
!       zii(c)  = 1000._r8     ! mars
    ...
    do fp = 1,num_nolakep
       p = filter_nolakep(fp)
       c = pcolumn(p)
!       thm(p)  = forc_t(c) + 0.0098_r8*forc_hgt_t_pft(p)  ! mars
        thm(p)  = forc_t(c) + SHR_CONST_DLAPSE*forc_hgt_t_pft(p) ! mars
!       write(*,*) "thm:", forc_t(c), SHR_CONST_DLAPSE, forc_hgt_t_pft(p), SHR_CONST_DLAPSE*forc_hgt_t_pft(p)
!       write(*,*) "thm", thm(p) ! mars
