...
! !LOCAL VARIABLES:
     ...
     real(r8)  q4(4,i1:i2,km)
      ...
      integer i, k, kk, kl, k0(i1:i2,0:kn+1), iq, k0found   ! ETW_AC_PATCH_2026_08_21
      real(r8)    pl, pr, qsum, qsumk(i1:i2,kn), delp, esl
...
!  Guard the level search, matching map1_ppm. Without k0found, a failed search
!  leaves k0(i,k) at a stale value and this routine silently returns garbage.
!  mapn_ppm_tracer is the constituent remapper carrying SO2, H2O and the CARMA
!  bins, so a corrupt column propagates into the tracer fields unnoticed.
!  Aborting loudly is the correct behaviour.
            k0found = -1
            do kk = k0(i,k-1), km
               if (pe2(i,k) <= pe1(i,kk+1)) then
                  k0(i,k) = kk
            ...
            enddo
            if (k0found .lt. 0) then
               write(iulog,*) 'mapz error (mapn) - k0found i j k = ', &
                  k0found, i, j, k
               call endrun('MAPZ_MODULE')
               return
            endif
