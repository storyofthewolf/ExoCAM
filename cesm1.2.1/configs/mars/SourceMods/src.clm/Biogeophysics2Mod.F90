...
! !USES:
    ...
    use clm_varcon        , only : hvap, cpair, grav, vkc, tfrz, sb, &
                                   ...
                                   istice_mec
    use clm_varcon        , only : istcrop, zvir
    use clm_varpar        , only : nlevsno, nlevgrnd, max_pft_per_col
    use SoilTemperatureMod, only : SoilTemperature
    use subgridAveMod     , only : p2c
    use CO2FrostMod       , only : CO2Frost  ! Mars
    use shr_const_mod     , only : SHR_CONST_PPATRIP, SHR_CONST_RGAS, SHR_CONST_KVISC  ! Mars
    use QSatMod           , only : QSat ! Mars

    use exoplanet_mod     , only : do_exo_condense_co2
    use exo_clm_condense  , only : exo_clm_co2srfice
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(r8), pointer :: forc_pbot(:)       !atmospheric pressure (Pa)         ! Mars
    real(r8), pointer :: forc_t(:)          !atmospheric temperature (kelvin)  ! Mars
    real(r8), pointer :: forc_rho(:)        !density (kg/m**3)                 ! Mars
    real(r8), pointer :: forc_q(:)          !atmospheric specific humidity(kg/kg)  ! Mars
    real(r8), pointer :: frac_sno(:)        !fraction of ground covered by snow (0 to 1)  ! Mars
...
! 
    ...
    real(r8), pointer :: co2ice_lh(:)       !co2 latent heat flux (W/m**2) [+ to grnd] ! mars
!
!
...
!
    ...
    real(r8) :: e                      ! water vapor pressure [Pa]   ! mars
    real(r8) :: qsatg                  ! saturated humidty [kg/kg]  ! mars
    real(r8) :: degdT                  ! d(eg)/dT  ! mars
    real(r8) :: qsatgdT                ! d(qsatg)/dT  ! mars
    real(r8) :: d                      ! diffusion coefficient of H2O in CO2 [cm2/s]   ! mars
    real(r8) :: rhos                   ! saturation vapor density [g/cm3]   ! mars
    real(r8) :: rho                    ! vapor density [g/cm3]   ! mars
    real(r8) :: rho_ratio              ! ratio in delta density over density [unitless]   ! mars
    real(r8) :: vis                    ! kinematic visocity [cm2/s]   ! mars
    real(r8) :: qflx_evap_conv         ! free convective mass loss [kg/m2/s=mm/s]   ! mars
...
!write(*,*) "num_nolakep", num_nolakep
!write(*,*) "land model, co2 condense", do_exo_condense_co2
    if (do_exo_condense_co2) then 
!      call CO2Frost(lbc, ubc, lbp, ubp, &   ! mars
!                               num_nolakec, filter_nolakec, &
!                               num_nolakep, filter_nolakep)

       call exo_clm_co2srfice(lbc, ubc, lbp, ubp, &   ! mars
                               num_nolakec, filter_nolakec, &
                               num_nolakep, filter_nolakep)
    ...
    do fc = 1,num_nolakec
       ...
       if (abs(tinc(c)) > 40.) then  ! mars
         write(6,*)'large soil temp difference:tinc=',tinc(c), &
        't_now=',t_soisno(c,j),'t_bef',tssbef(c,j)
       endif
...
!       ! mars error checking
       if (eflx_sh_grnd(p) < -300.) then
          write(6,*)'shflx very low in biogeophys2 L283:shflx=',eflx_sh_grnd(p),'tinc=',tinc(c),'cgrnds=',cgrnds(p)
          endif 
! this catch below was commented out in Richard/Torie's version
       if (qflx_evap_soi(p)*htvp(c) > 100.) then 
         write(6,*)'qflx high in bio2: qflx*htvp=',qflx_evap_soi(p)*htvp(c),'tinc*cgrndl',tinc(c)*cgrndl(p)
       endif ! error loop mars
    end do
    ...
    do fp = 1,num_nolakep
       ...
       ! Right now qflx_evap_soi has unrealistically high values because the water in the model
       ! is less than the lower bound leading to extra latent heat, which gets added to eflx_sh_grnd.
       ! This part of the code has been removed so this extra lh is not added.
       ! It probably should be put back in runs with more water in the model. (Urata 10/27/09)

       if (egirat(c) < 1.0_r8) then
          save_qflx_evap_soi = qflx_evap_soi(p)
          qflx_evap_soi(p) = qflx_evap_soi(p) * egirat(c)
!          eflx_sh_grnd(p) = eflx_sh_grnd(p) + (save_qflx_evap_soi - qflx_evap_soi(p))*htvp(c)  ! comment out? original
          eflx_lh_grnd(p) = eflx_lh_grnd(p) + (save_qflx_evap_soi - qflx_evap_soi(p))*htvp(c)  ! mars
       end if
       ! mars error handling
      if (abs(eflx_sh_grnd(p)) > 1000.) then
        write(6,*)'shflx very low in biogeophys2:shflx=',eflx_sh_grnd(p),'qflx=', & 
        qflx_evap_soi(p),'htvp=',htvp(c),'save_qflx=',save_qflx_evap_soi
      endif 
 ...
 ! write(*,*) "biog2, eflx_soil_grnd", eflx_soil_grnd(p), "sabg", sabg(p), "dlrad", dlrad(p), &
 !                             "1-fac_vegnosno", (1-frac_veg_nosno(p)), "emg", emg(c), "forc_lwrad", forc_lwrad(g), &
 !                             "emg", emg(c), "emg*sb*t^3*tbef", emg(c)*sb*tssbef(c,j)**3*tssbef(c,j), "tinc", tinc(c), &
 !                             "eflx_sh_grnd", eflx_sh_grnd(p), "qflx_evap_soi*htvp", qflx_evap_soi(p)*htvp(c)
       ...
       if ((abs(qflx_evap_tot(p)) < 1.e-14_r8) .or.(isnan(qflx_evap_tot(p)))) then  ! mars
         !write(6,*) 'qflx_tot is:',qflx_evap_tot(p)  ! mars
         qflx_evap_tot(p) = 0._r8  ! mars
         qflx_evap_veg(p) = 0._r8  ! mars
         qflx_evap_soi(p) = 0._r8  ! mars
       endif ! mars
       ! why am I adding energy here ?? 
       ! co2ice_lh gets canceled out in surface energy balance equation :-?
       eflx_lh_tot(p)= hvap*qflx_evap_veg(p) + htvp(c)*qflx_evap_soi(p) - co2ice_lh(p) ! mars
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
	  ...
	  if ((h2osoi_liq(c,j)+h2osoi_ice(c,j)) > 0.) then
	     ...
	     qflx_evap_grnd(p) = 0.
          ...
          if ((t_grnd(c) < tfrz) .or. (forc_pbot(g) < SHR_CONST_PPATRIP)) then ! mars
       ...
       qflx_sub_snow(p) = qflx_sub_snow(p)  ! mars why is this here
    ...
    do fp = 1,num_nolakep
       p = filter_nolakep(fp)
       c = pcolumn(p)
       errsoi_pft(p) = eflx_soil_grnd(p) - xmf(c) + co2ice_lh(p) ! mars
!write(*,*)  "p, errsoi_pft(p)", p, errsoi_pft(p), eflx_soil_grnd(p), xmf(c), co2ice_lh(p) ! mars   
...
!write(*,*)    "p, errsoi_pft 2", p, errsoi_pft(p), (t_soisno(c,j)-tssbef(c,j))/fact(c,j), t_soisno(c,j)-tssbef(c,j), fact(c,j)
          end if
       end do
    end do
    ...
    do fp = 1,num_nolakep
       ...
       ! mars
       if ((eflx_lwrad_out(p) < 0.) .or. (eflx_lwrad_out(p) > sb*400.**4))then
          write(6,*)'bad lw_out in bio2:ulrad=',ulrad(p),'sb=', sb, 'emg=',emg(c),'forc_lw=', &
                     forc_lwrad(g),'tssbef=',tssbef(c,j),'tinc=',tinc(c), &
                    'emg*forc_lw=',(1-frac_veg_nosno(p))*(1.-emg(c))*forc_lwrad(g), &
                    'emg*tssbef=',(1-frac_veg_nosno(p))*emg(c)*sb * tssbef(c,j)**4, &
                    'emg*bef*tinc=',4.*emg(c)*sb*tssbef(c,j)**3*tinc(c), &
                    'eflx_lwrad=',eflx_lwrad_out(p),'t_soi=',t_soisno(c,j) !, &
!                   'lat_deg=',latdeg(c)
        endif
          ! mars 
          ! There are problems with low temperatures, and large tinc differences resulting in negative
          ! lw flux outs.  This is a band-aid to try and get around this by just having the current sfc_t
          ! instead of the difference.
!         if (eflx_lwrad_out(p) < 0.) then        ! mars
!           eflx_lwrad_out(p) = ulrad(p) &  ! mars 
!                             + (1-frac_veg_nosno(p))*(1.-emg(c))*forc_lwrad(g) &  ! mars
!                            + (1-frac_veg_nosno(p))*emg(c)*sb * t_soisno(c,j)**4 ! mars
!         endif  ! mars
    end do
