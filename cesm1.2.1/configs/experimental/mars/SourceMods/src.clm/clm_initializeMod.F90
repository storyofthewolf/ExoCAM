...
!EOP
    ...
    real(r8) :: frac_day, frac_daym1   ! mars
    real(r8) :: frac_year, frac_yearm1  ! mars
!----------------------------------------------------------------------
...
!$OMP END PARALLEL DO
    ...
    if (get_nstep() == 0 .or. nsrest == nsrStartup) then
       ...
       if (finidat == ' ' .or. do_initsurfalb) then
          ...
          call get_curr_calday_rotation(frac_daym1, frac_yearm1,offset=-int(dtime)) ! mars
          call t_startf('init_orbd2')
          call shr_orb_decl( frac_yearm1*exo_porb, eccen, mvelpp, lambm0, obliqr, declinm1, eccf ) ! mars
          call t_stopf('init_orbd2')
          
          call t_startf('init_orbSA')
          call initSurfAlb( frac_year*exo_porb, declin, declinm1 ) ! mars
