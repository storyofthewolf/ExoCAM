...
! !USES:
  ...
  use shr_const_mod,      only: shr_const_pstd, shr_const_g
...
! dry surface pressure
      ...
      parameter (drym = SHR_CONST_PSTD)        ! For US NAVY 10-min terrain
#else
      !parameter (drym = 98288.0_r8)           ! For USGS terrain
      parameter (drym = SHR_CONST_PSTD)           ! For USGS terrain
#endif
    ...
    if (aqua_planet) then
       ...
       drym_loc = SHR_CONST_PSTD-245._r8
    end if
...
!$omp  parallel do private(i,j)
 ...
 CPP_PRT_PREFIX write(iulog,*) 'Total Precipitable Water =', (psm0-psdry)/SHR_CONST_G, '(kg/m**2)'
