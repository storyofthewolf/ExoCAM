...
! !USES:
    ...
    use clm_varcon         , only : hvap, hsub, hfus, cpair, cpliq, cpice, &
                                    tkwat, tkice, tkco2ice, &
                                    sb, vkc, grav, denh2o, tfrz, spval
    use QSatMod            , only : QSat
    use FrictionVelocityMod, only : FrictionVelocity, MoninObukIni
    use TridiagonalMod     , only : Tridiagonal
    use shr_const_mod      , only : SHR_CONST_DLAPSE, SHR_CONST_DENSITYCO2FR, &
                                    SHR_CONST_CO2_EMIS, SHR_CONST_CO2ICE_THICK
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(r8), pointer :: emg(:)             ! surface emissivity for use in land model
    real(r8), pointer :: srf_emiss(:)       ! surface emissivity for passsing to coupler
!
!
...
!-----------------------------------------------------------------------
    ...
    do fc = 1, num_lakec
       ...
       dzsur(c) = dz(c,1) + snowdp(c) + co2z

       ! Set surface emissivity pointers

       if (co2z > SHR_CONST_CO2ICE_THICK) then
         emg(c) = SHR_CONST_CO2_EMIS
       else
         emg(c) = 1.0
       endif
       srf_emiss(c) = emg(c)
       !write(*,*) "lake emiss", c, srf_emiss(c), emg(c)     
    ...
    do fp = 1, num_lakep
       ...
       obuold(p) = 0._r8
       displa(p) = 0._r8
       thm(p) = forc_t(g) + SHR_CONST_DLAPSE*forc_hgt_t_pft(p)   ! intermediate variable
    ...
    ITERATION : do while (iter <= niters .and. fncopy > 0)
       ...
       do fp = 1, fncopy
          ...
          ax  = sabg(p) + emg(c)*forc_lwrad(g) + 3._r8*stftg3(p)*tgbef(c) &
    ...
    do fp = 1, num_lakep
       ...
       eflx_lwrad_out(p) = (1._r8-emg(c))*forc_lwrad(g) + stftg3(p)*(-3._r8*tgbef(c)+4._r8*t_grnd(c))
