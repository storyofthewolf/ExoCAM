...
! !USES:
    ...
    use mct_mod
    use ESMF
    use exoplanet_mod, only: exo_ndays, exo_porb  ! mars  
    implicit none
!
! !ARGUMENTS:
...
! !LOCAL VARIABLES:
    ...
    real(r8) :: frac_day, frac_dayp1   ! mars                                                                                             
    real(r8) :: frac_year, frac_yearp1  ! mars  
...
!MARS FIX THIS?
 ...
 !      call shr_orb_decl( calday     , eccen, mvelpp, lambm0, obliqr, declin  , eccf )  ! orig, comment out mars
 !      call shr_orb_decl( nextsw_cday, eccen, mvelpp, lambm0, obliqr, declinp1, eccf )  ! orig, comment out mars

!write(*,*) "lnd_comp_mct: cal", calday, nextsw_cday
!write(*,*), "declin", declin
!write(*,*), "declinp1", declinp1
!write(*,*), "---------------------------------------"


       call get_curr_calday_rotation(frac_day, frac_year) ! mars  
       call get_curr_calday_rotation(frac_dayp1, frac_yearp1, offset=int(dtime)*exo_rad_step) ! mars  
       call shr_orb_decl( frac_year*exo_porb, eccen, mvelpp, lambm0, obliqr, declin, eccf )  ! mars  
       call shr_orb_decl( frac_yearp1*exo_porb, eccen, mvelpp, lambm0, obliqr, declinp1, eccf ) ! mars

!write(*,*) "lnd_comp_mct: frac", frac_year, frac_yearp1, frac_day, frac_dayp1
!write(*,*) "frac, declin:", declin
!write(*,*) "frac, declinp1:", declinp1
!write(*,*), "---------------------------------------"
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
       ...
       l2x_l%rAttr(index_l2x_Sl_co2dp,i)           =  clm_l2a%co2dp(g) ! mars
       l2x_l%rAttr(index_l2x_Sl_co2_mass_change,i) =  clm_l2a%co2_mass_change(g) ! mars
       l2x_l%rAttr(index_l2x_Sl_srf_emiss,i)       =  clm_l2a%srf_emiss(g) ! mars
       ...
       l2x_l%rAttr(index_l2x_Fall_lat,i)           = -clm_l2a%eflx_lh_tot(g)   ! water only latent heat flux
       l2x_l%rAttr(index_l2x_Fall_lat_co2,i)       = -clm_l2a%eflx_lh_co2(g)   ! co2 latent heat flux
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
        ...
        a2l%forc_co2srf_snow(g)       = x2l_l%rAttr(index_x2l_Faxa_co2srf_snow,i)      ! MARS_ETW
        a2l%forc_co2srf_cond(g)       = x2l_l%rAttr(index_x2l_Faxa_co2srf_cond,i)      ! MARS_ETW
