...
! !USES:
    ...
    use clm_varcon         , only : hvap, hsub, hfus, cpair, cpliq, cpice, tkwat, tkice, &
                                    sb, vkc, grav, denh2o, tfrz, spval, zvir
...
!-----------------------------------------------------------------------
    ...
    do fc = 1, num_lakec
       ...
       thv(c) = forc_th(g)*(1._r8+zvir*forc_q(g))     ! virtual potential T
    end do
    ...
    do fp = 1, num_lakep
       ...
       dthv     = dth(p)*(1._r8+zvir*forc_q(g))+zvir*forc_th(g)*dqh(p)
    ...
    ITERATION : do while (iter <= niters .and. fncopy > 0)
       ...
       do fp = 1, fncopy
          ...
          !not used
          !dthv=dth(p)*(1.+zvir*forc_q(g))+zvir*forc_th(g)*dqh(p)
