...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  public :: SoilThermProp   ! Set therm conductivities and heat cap of snow/soil layers!
...
!
    ...
    real(r8), pointer :: thermal_inertia(:) ! soil thermal inertia factor for soil heating(I*sqrt(pi/P))  ! mars
    real(r8), pointer :: forc_pbot(:)       ! bottom level pressure (Pa)  ! mars
...
!
    ...
    real(r8) :: egsmax(lbc:ubc)      !max. evaporation which soil can provide at one time step     ! mars
    integer  :: n,nsub,n2                       !index and limit for substeps, default is 10 if dT/dz is large ! mars 
    real(r8) :: t_soisno_tmp(lbc:ubc,-nlevsno+1:nlevgrnd)            !temporary array to hold old soisno temperatures ! mars 
                                                                                !in case substepping is required  ! mars
...
!-----------------------------------------------------------------------
    ...
    thermal_inertia  => cps%thermal_inertia  ! mars
...
!   n2=0 ! mars 
    ...
    !adding substepping for when dT/dz is too big. ! mars 
    !t_soisno_tmp(lbc:ubc,-nlevsno+1:nlevsoi)=t_soisno(lbc:ubc,-nlevsno+1:nlevsoi) ! mars

!10 do n=1,nsub                                                                                                                                                                     
     dtime = dtime/real(nsub) !mars
          ...
          if ( pi <= npfts(c) ) then
             ...
             if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
                if (ltype(l) /= isturb) then
                  ! don't add latent heat if maximum water available is below the threshold.  mars
                  ! this is to get around the issue of "fake" water flux. mars

                  if (forc_pbot(g) > 1.e4) then  ! mars
                    eflx_gnet(p) = sabg(p) + dlrad(p) &   ! net energy into the ground
                                  + (1.0_r8-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) - lwrad_emit(c) &
                    ...
                    eflx_gnet(p) = sabg(p) + dlrad(p) &   ! net energy into the ground
                       ...
                       if (abs(qflx_evap_soi(p)*htvp(c)) > 1000.) then 
                               write(6,*)'lhflx high in soiltemp:','qflx*htvp:',qflx_evap_soi(p)*htvp(c), &
                               'qflx_evap_soi=', qflx_evap_soi(p), 'htvp=', htvp(c), &
                               'shflx=',eflx_sh_grnd(p),'gnet=',eflx_gnet(p), & 
                               'qflx=',qflx_evap_soi(p),'sabg=',sabg(p),'dlrad=',dlrad(p), & !'temp1=',temp1(c), & 
                               'forc_lw=',(1-frac_veg_nosno(p))*emg(c)*forc_lwrad(g),'egsmax=',egsmax(c) 
                       endif ! mars
       ...
       do fc = 1,num_nolakec
          ...
          if (j >= snl(c)+1) then   
             if (j == snl(c)+1) then
                ...
                fn(c,j) = tk(c,j)*(t_soisno(c,j+1)-t_soisno(c,j))/(z(c,j+1)-z(c,j))  ! heat diffusion through layer interface
...
! !USES:
    ...
    use clm_varcon,     only : denh2o, denice, &
                               tfrz, tkwat, tkice, tkair, tkco2ice, &
                               cpice,  cpliq,  istice, istice_mec, istwet, &
                               icol_roof, icol_sunwall, icol_shadewall, &
                               icol_road_perv, icol_road_imperv
    use clm_varpar,     only : nlevsno, nlevgrnd, nlevurb, nlevsoi
    use shr_const_mod,  only : SHR_CONST_DENSITYCO2FR, SHR_CONST_DENSITYCO2ICE, &
                               SHR_CONST_CPCO2ICE, SHR_CONST_CO2_TI, &
                               SHR_CONST_CO2ICE_THICK
...
!
    real(r8), pointer :: thermal_conduct(:) ! soil thermal conductivity from inertia  ! mars
    real(r8), pointer :: thermal_inertia(:) ! soil thermal inertia factor for soil heating(I*sqrt(pi/P)) ! mars
...
!-----------------------------------------------------------------------
    ...
    thermal_conduct  => cps%thermal_conduct  ! mars, themal conductivty = I^2/(rho*cp)
    thermal_inertia  => cps%thermal_inertia ! mars, thermal inertia
       ...
       do fc = 1, num_nolakec
          ...
          if (j >= 1) then  ! soil levels    
             ...
             elseif (ltype(l) /= istwet .AND. ltype(l) /= istice  &
                ...
                satw = min(1._r8, satw)
                if (satw > .1e-6_r8) then   ! if soi. water is present
                   ...
                   if ((thermal_conduct(c) > 0.) ) then      ! override previous calculation if thermal inertia/conductivity present of surfdat
                      !thk(c,j) = dke*dksat + (1.-dke)*thermal_conduct(c)                  !mars 
                      thk(c,j) = thermal_conduct(c)          ! since the thermal inertia field includes subsurface ice, ! mars
                                                             ! just use the thermal conductivity from that.  It can't be used  ! mars
                                                             ! for changing subsurface conditions though.  ! mars
                      ! can set H2O dependence similar to Turbet et al. 2022, Wang et al. (2016)
                   else                                      ! mars
                      thk(c,j) = dke*dksat + (1.-dke)*tkdry(c,j)  ! original, no mars
                   endif
                else    ! dry soil
                  if ((thermal_conduct(c) > 0.) ) then   ! mars
                     thk(c,j) = thermal_conduct(c)       ! mars
                   else                           ! mars 
                     thk(c,j) = tkdry(c,j)        ! original, no mars
                   endif
                endif
          ...
          if (snl(c)+1 < 1 .AND. (j >= snl(c)+1) .AND. (j <= 0)) then  
             bw = (h2osoi_ice(c,j)+h2osoi_liq(c,j))/dz(c,j)   ! partial density of water
   ...
   ! CO2 ice thermal conductivity
   ! CO2 ice is assumed to overlay soil and snow 
   ! and is incoporated into top layer
   do fc = 1,num_nolakec
      c = filter_nolakec(fc)
      co2z = co2dp(c) / SHR_CONST_DENSITYCO2FR
      if (co2z > SHR_CONST_CO2ICE_THICK) then
        thk(c,snl(c)+1) = tkco2ice
        !write(*,*) "co2z thk", thk(c,snl(c)+1)
      endif
   end do
   ...
   do fc = 1,num_nolakec
      ...
      co2z = co2dp(c) / SHR_CONST_DENSITYCO2FR
      if (co2z > SHR_CONST_CO2ICE_THICK) then
        ! this should be a weighted average between the H2O and CO2 snow amounts
        ! or additional layering depending upon which condenses last.
        ! However, CO2, H2O ice mixing is not presently included
        cv(c,snl(c)+1) = SHR_CONST_CPCO2ICE *  SHR_CONST_DENSITYCO2FR * co2z
        !write(*,*) "co2z cv", cv(c,snl(c)+1)
      endif
   end do
