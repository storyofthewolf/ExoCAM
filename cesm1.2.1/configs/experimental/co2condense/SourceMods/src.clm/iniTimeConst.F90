...
!
  ...
  real(r8), pointer :: thermal_conduct(:) ! soil thermal conductivity from inertia input, 2D  ! mars
  real(r8), pointer :: thermal_inertia(:) ! soil thermal inertia factor for soil heating from input, 2D  ! mars
  ...
  real(r8), pointer :: z0m_ini(:)         ! roughness length over ground,momentum [m]   ! mass
...
!EOP
  ...
  real(r8),pointer :: soir2d(:)        ! read in - soil roughness length   ! mars
  ...
  real(r8),pointer :: thm_inertia2d(:) ! read in thermal interia !mars         
...
!------------------------------------------------------------------------
  ...
  allocate(soic2d(begg:endg), soir2d(begg:endg), gti(begg:endg))
  allocate(sand3d(begg:endg,nlevsoi), clay3d(begg:endg,nlevsoi))
  allocate(organic3d(begg:endg,nlevsoi))
  allocate(thm_inertia2d(begg:endg))  ! mars, allocate thermal intertia input
...
if(masterproc) write(iulog,*)"mxsoli_color", mxsoil_color
  ! Read fmax
...
if(masterproc) write(iulog,*)"soil color", soic2d
  ! Read in soil roughness

  call ncd_io(ncid=ncid, varname='ROUGHNESS', flag='read', data=soir2d, dim1name=grlnd, readvar=readvar)
  if (.not. readvar) call endrun( trim(subname)//' ERROR: ROUGHNESS NOT on surfdata file' ) 
if(masterproc) write(iulog,*)"roughness", soir2d
  ...
  !! added for Mars
  call ncd_io(ncid=ncid, varname='THM_INERTIA', flag='read', data=thm_inertia2d, dim1name=grlnd, readvar=readvar)
  if (.not. readvar) call endrun( trim(subname)//' ERROR: THM_INERTIA not on surfdata file' ) 
  
if(masterproc) write(iulog,*)"thm_inertia2d", thm_inertia2d

  if (masterproc) then
     write(iulog,*) 'Successfully read thermal inertia THM_INERTIA'
     write(iulog,*)
  endif
  ...
  else if (mxsoil_color == 12) then
     ! Mars - 12 soil classes defined in surfdata files
     ! saturated soil albedos for 12(8 in old) color classes: 1=vis, 2=nir 
     albsat(1:12,1) = (/0.12, 0.11, 0.10, 0.09, 0.08, 0.07, 0.06, 0.05, 0.13, 0.14, 0.15, 0.16/)
  ...
  ! --------------------------------------------------------------------
   ...
   do c = begc, endc
      ...
      ! Soil color, read in from surfdat file
      isoicol(c) = soic2d(g)

if(masterproc) write(*,*) "iniTimeConst, color", soic2d(g)       

      ! Soil roughness length, read in from surfdat file
      z0m_ini(c) = soir2d(g)   ! mars
if(masterproc) write(*,*) "iniTimeConst, z0", z0m_ini(c), soir2d(g)       
      ! Soil thermal inertia factor for soil heating 
      ! what is the factor of sqrt(SHR_CONST_PI/SHR_CONST_CDAY) for?
!      ithincol_i(c) = thin2d(g)*sqrt(SHR_CONST_PI/SHR_CONST_CDAY)  ! mars
      thermal_inertia(c) = thm_inertia2d(g)*sqrt(SHR_CONST_PI/SHR_CONST_CDAY)  ! mars

if(masterproc) write(*,*) "iniTimeConst, thm", thm_inertia2d(g)       
      ! Soil thermal conductivity from inertia
      !factor in front of thin2d is to test effect of changing thermal inertia
      !ithincol(c) = ((SHR_CONST_THINFAC*thm_inertia2d(g))**2)/(SHR_CONST_SOILBD*SHR_CONST_SOILCP)  !mars
      thermal_conduct(c) = ((SHR_CONST_THINFAC*thm_inertia2d(g))**2)/(SHR_CONST_SOILBD*SHR_CONST_SOILCP)  !mars
      ...
      if (ltype(l)==istdlak .or. ltype(l)==istwet .or. ltype(l)==istice .or. ltype(l)==istice_mec) then
         ...
         do lev = 1,nlevgrnd
            ...
            if(do_exo_soilcp) then ! sand properties are used for everything but heat capacity
              csol(c,lev) = SHR_CONST_SOILBD * SHR_CONST_SOILCP  ! J/(m3 K)  ! Mars
            endif 
