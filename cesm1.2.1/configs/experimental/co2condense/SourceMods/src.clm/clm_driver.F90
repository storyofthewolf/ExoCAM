...
!      -> CO2Frost        add any CO2 frost which might have formed on the surface, Mars
...
! !USES:
  ...
  use shr_const_mod       , only : SHR_CONST_DENSITYCO2FR ! mars 
  use clmtype
...
!-----------------------------------------------------------------------
  ...
  do nc = 1,nclumps
     ...
     ! ===========================================================================
     ! Determine CO2 condensation onto surface.  Calculates CO2 snow mass and 
     ! latent heating/cooling from direct condensation, sublimation, and snowfall
     ! collection.  
     ! ===========================================================================

     if (do_exo_condense_co2) then
       call t_startf('exo_clm_co2srfice')
       call exo_clm_co2srfice(begc, endc, begp, endp, &
                              filter(nc)%num_nolakec, filter(nc)%nolakec, &
                              filter(nc)%num_condensep, filter(nc)%condensep)
       call t_stopf('exo_clm_co2srfice')
     endif   
     ...
     do c = begc,endc
        ...
        if (itypelun(l) == isturb) then
          ...
          ! cps%frac_sno(c) = 0.0_r8           ! mars comment out
          ...
          ! endif
          cps%frac_sno(c) = cps%snowdp(c) / (10.*zlnd + cps%snowdp(c) + &  ! mars
          cps%co2dp(c)/SHR_CONST_DENSITYCO2FR)  ! mars
        end if
     end do


     ! ============================================================================
     ! ! Fraction of soil covered by co2 frost (patterned after snow fraction) 
     ! ============================================================================
     do c = begc,endc
         if(cps%co2dp(c)/SHR_CONST_DENSITYCO2FR .gt. 0.0_r8)  then 
           cps%frac_co2(c) = ( cps%co2dp(c)/SHR_CONST_DENSITYCO2FR ) / &    
                             (10.*cps%z0m_ini(c) + cps%snowdp(c) + cps%co2dp(c)/SHR_CONST_DENSITYCO2FR)
         else
           cps%frac_co2(c) = 0.0
        endif
     end do
