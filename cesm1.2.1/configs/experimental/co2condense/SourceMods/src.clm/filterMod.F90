...
! !PUBLIC TYPES:
  ...
  type clumpfilter
     ...
     integer, pointer :: condensep(:)        ! condense filter (pgts) 
     integer :: num_condensep                ! number of pfts in condense filter 
...
!------------------------------------------------------------------------
    ...
    do nc = 1, nclumps
       ...
       allocate(filter(nc)%condensep(endp-begp+1))
    end do
    !$OMP END PARALLEL DO
...
! LOCAL VARAIBLES:
    ...
    integer :: fc          ! condense filter index
...
!------------------------------------------------------------------------
    ...
    ! Create condense filters at pft-level 
    ! Filter will only be active if weight of pft wrt gcell is nonzero
...
!          if (lun%itype(l) == istsoil .or. lun%itype(l) == istcrop) then
             fc = fc + 1
             filter(nc)%condensep(fc) = p
!          end if
       end if
    end do
    filter(nc)%num_condensep = fc
