...
type dyn_import_t
     ...
     real(r8), dimension(:,:,:  ), pointer     :: du3sdt ! U-winds (staggered) tendency
     real(r8), dimension(:,:,:  ), pointer     :: dv3sdt ! V-winds (staggered) tendency
     real(r8), dimension(:,:,:  ), pointer     :: dptdt  ! Temperature tendency
...
!   CB  15.05.01:  Updated Dyn/Phys stepping !! C.Bardeen
!
!----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   allocate( dyn_in%du3sdt(  I1:IN,J1:JN,K1:KN  ), stat=ierror )        !! C. Bardeen
   if ( ierror /= 0 ) call endrun('DYN_COMP ALLOC error: array DU3SDT') !! C. Bardeen
   allocate( dyn_in%dv3sdt(  I1:IN,J1:JN,K1:KN  ), stat=ierror )        !! C. Bardeen
   if ( ierror /= 0 ) call endrun('DYN_COMP ALLOC error: array DV3SDT') !! C. Bardeen
   ...
   allocate( dyn_in%dptdt(   I1:IN,J1:JN,K1:KN  ), stat=ierror )       !! C. Bardeen
   if ( ierror /= 0 ) call endrun('DYN_COMP ALLOC error: array DPTDT') !! C. Bardeen
...
!
   ...
   dyn_in%pkz = inf
   dyn_in%delp = inf
!   dyn_in%du3sdt = inf    !! C.Bardeen
!   dyn_in%dv3sdt = inf    !! C.Bardeen 
   dyn_in%du3sdt = 0._r8   !! C.Bardeen
   dyn_in%dv3sdt = 0._r8   !! C.Bardeen
!   dyn_in%dptdt = inf     !! C.Bardeen
   dyn_in%dptdt = 0._r8    !! C.Bardeen
...
! Variables from the dynamics interface (import or export)
   ...
   real(r8), pointer :: dptdtxy(:,:,:) ! scaled (virtual) potential temperature tendency   !! C.Bardeen
   ...
   real(r8), pointer :: dudtxy(:,:,:) ! u wind tendencies, staggered grid  !! C.Bardeen
   real(r8), pointer :: dvdtxy(:,:,:) ! v wind tendencies, staggered grid  !! C.Bardeen
...
!  YZ variables currently allocated on stack... should they be on the heap?
   ...
   real(r8) :: v(dyn_state%grid%im,   &
                 ...
                 dyn_state%grid%kfirst:dyn_state%grid%klast)
   real(r8) :: dudt(dyn_state%grid%im,   &                              !! C.Bardeen
                 dyn_state%grid%jfirst-dyn_state%grid%ng_d:dyn_state%grid%jlast+dyn_state%grid%ng_s,&
                 dyn_state%grid%kfirst:dyn_state%grid%klast)
   real(r8) :: dvdt(dyn_state%grid%im,   &                              !! C.Bardeen
                 dyn_state%grid%jfirst-dyn_state%grid%ng_s:dyn_state%grid%jlast+dyn_state%grid%ng_d,&
                 dyn_state%grid%kfirst:dyn_state%grid%klast)
   real(r8) :: pt(dyn_state%grid%im,  &
                  dyn_state%grid%jfirst-dyn_state%grid%ng_d:dyn_state%grid%jlast+dyn_state%grid%ng_d,&
                  dyn_state%grid%kfirst:dyn_state%grid%klast)
   real(r8) :: dptdt(dyn_state%grid%im,  &                              !! C.Bardeen
                  dyn_state%grid%jfirst-dyn_state%grid%ng_d:dyn_state%grid%jlast+dyn_state%grid%ng_d,&
                  dyn_state%grid%kfirst:dyn_state%grid%klast)
...
! C.-C. Chen, omega calculation
  ...
  real(r8) :: &
   ...
   uxy      => dyn_in%u3s
   vxy      => dyn_in%v3s
   dudtxy   => dyn_in%du3sdt  !! C.Bardeen
   dvdtxy   => dyn_in%dv3sdt  !! C.Bardeen
   t3xy     => dyn_in%t3
   ptxy     => dyn_in%pt
...
! These express the physics tendencies, so don't change during dynamics.   !! C.Bardeen
           if ((substep_uv) .and. (iv .eq. 1)) then

              call mp_sendirr( grid%commxy, grid%dudtxy_to_dudt%SendDesc,                &
                               grid%dudtxy_to_dudt%RecvDesc, dudtxy, dudt,               &
                               modc=grid%modc_dynrun )
              call mp_recvirr( grid%commxy, grid%dudtxy_to_dudt%SendDesc,                &
                               grid%dudtxy_to_dudt%RecvDesc, dudtxy, dudt,               &
                               modc=grid%modc_dynrun )

              call mp_sendirr( grid%commxy, grid%dvdtxy_to_dvdt%SendDesc,                &
                               grid%dvdtxy_to_dvdt%RecvDesc, dvdtxy, dvdt,               &
                               modc=grid%modc_dynrun )
              call mp_recvirr( grid%commxy, grid%dvdtxy_to_dvdt%SendDesc,                &
                               grid%dvdtxy_to_dvdt%RecvDesc, dvdtxy, dvdt,               &
                               modc=grid%modc_dynrun )
           end if
...
! These express the physics tendencies, so don't change during dynamics.   !! C.Bardeen
           if ((substep_t) .and. (iv .eq. 1)) then
              call mp_sendirr( grid%commxy, grid%dptdtxy_to_dptdt%SendDesc,              &
                               grid%dptdtxy_to_dptdt%RecvDesc, dptdtxy, dptdt,           &
                               modc=grid%modc_dynrun )
              call mp_recvirr( grid%commxy, grid%dptdtxy_to_dptdt%SendDesc,              &
                               grid%dptdtxy_to_dptdt%RecvDesc, dptdtxy, dptdt,           &
                               modc=grid%modc_dynrun )
            end if
           ...
           do k = 1,km
              do j = jfirst,jlast
                 do i = 1,im
                    ...
                    if ((substep_uv) .and. (iv .eq. 1)) then  !! C.Bardeen
                       dudt(i,j,k) = dudtxy(i,j,k)
                       dvdt(i,j,k) = dvdtxy(i,j,k)
                    end if
                    ...
                    if ((substep_t) .and. (iv .eq. 1)) then   !! C.Bardeen
                       dptdt(i,j,k) = dptdtxy(i,j,k)
                    end if
...
!
!! C. Bardeen
! If doing substeping of uv or pt, then on the first iteration, the state needs
! to be reset to what it was prior to physics and then on each step an increment of
! the tendency is added to the field.
!
! Substepping u and v happens in the inner loop, so that you get the smallest increments
! possible tp sread out the physics tendency.
     if (substep_uv) then
        if (iv .eq. 1) then
           u(1:im,jfirst:jlast,kfirst:klast) = u(1:im,jfirst:jlast,kfirst:klast) - dudt(1:im,jfirst:jlast,kfirst:klast) * ndt
           v(1:im,jfirst:jlast,kfirst:klast) = v(1:im,jfirst:jlast,kfirst:klast) - dvdt(1:im,jfirst:jlast,kfirst:klast) * ndt
        end if

!        u(1:im,jfirst:jlast,kfirst:klast) = u(1:im,jfirst:jlast,kfirst:klast) + dudt(1:im,jfirst:jlast,kfirst:klast) * ndt / nv
!        v(1:im,jfirst:jlast,kfirst:klast) = v(1:im,jfirst:jlast,kfirst:klast) + dvdt(1:im,jfirst:jlast,kfirst:klast) * ndt / nv
     end if

! Substepping pt is done after the remapping, so that the vertical grid is known. Only the tendency
! on potential temperature due to changes in temperature are considered.
!
! Substepping pt happens on the outer loop, which is after vertical remapping.
     if (substep_t) then
        if (iv .eq. 1) then
           pt(1:im,jfirst:jlast,kfirst:klast) = pt(1:im,jfirst:jlast,kfirst:klast) - dptdt(1:im,jfirst:jlast,kfirst:klast) * ndt
        end if
        pt(1:im,jfirst:jlast,kfirst:klast) = pt(1:im,jfirst:jlast,kfirst:klast) + dptdt(1:im,jfirst:jlast,kfirst:klast) * ndt / nv
     end if
!! C.Bardeen !!
...
!! C.Bardeen
! If substepping u and v, then update the u and v fields with an increment from the
! physics tendency.
           if (substep_uv) then
              u(1:im,jfirst:jlast,kfirst:klast) = u(1:im,jfirst:jlast,kfirst:klast) + dudt(1:im,jfirst:jlast,kfirst:klast) * dt
              v(1:im,jfirst:jlast,kfirst:klast) = v(1:im,jfirst:jlast,kfirst:klast) + dvdt(1:im,jfirst:jlast,kfirst:klast) * dt
           end if
...
!-----------------------------------------------------------------------
   ...
   if ( associated(dyn_in%du3sdt) )  deallocate( dyn_in%du3sdt )  !! C. Bardeen
   if ( associated(dyn_in%dv3sdt) )  deallocate( dyn_in%dv3sdt )  !! C. Bardeen
   if ( associated(dyn_in%pe) )   deallocate( dyn_in%pe )
   if ( associated(dyn_in%pt) )   deallocate( dyn_in%pt )
   if ( associated(dyn_in%dptdt) )   deallocate( dyn_in%dptdt )  !! C. Bardeen
