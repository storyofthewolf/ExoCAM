...
!
    ...
    real(r8), pointer :: ithincol_i(:)      ! soil thermal inertia factor for soil heating(I*sqrt(pi/P))  ! mars
...
!-----------------------------------------------------------------------
    ...
    ithincol_i   => cps%ithincol_i  ! mars
...
!write(*,*)  "st2",  (1-frac_veg_nosno(p)), emg(c),forc_lwrad(g), lwrad_emit(c)
!write(*,*) "st3",    eflx_sh_grnd(p), qflx_evap_soi(p), htvp(c)
!write(*,*) "st3",    eflx_sh_grnd(p)
!write(*,*) "st3",    qflx_evap_soi(p)
!write(*,*) "st3",    htvp(c)
!write(*,*) "st",  dlrad(p)
!write(*,*) "st", sabg(p)
!write(*,*) "st", forc_pbot(g)
!write(*,*) "st", forc_pbot(g), sabg(p), dlrad(p)
                  if (forc_pbot(g) > 1.e4) then  ! mars
                    eflx_gnet(p) = sabg(p) + dlrad(p) &
                                  + (1.0_r8-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) - lwrad_emit(c) &
                                  - (eflx_sh_grnd(p)+qflx_evap_soi(p)*htvp(c)) ! + co2ice_lh(p) ! mars
                  else
                    eflx_gnet(p) = sabg(p) + dlrad(p) &
                                  + (1.0_r8-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) - lwrad_emit(c) &
                                  - (eflx_sh_grnd(p)) !+ co2ice_lh(p) ! mars
                  endif !/mars
                   ...
                   eflx_gnet(p) = sabg(p) + dlrad(p)  &
                                  ...
                                  + eflx_wasteheat_pft(p) + eflx_heat_from_ac_pft(p) + eflx_traffic_pft(p) !+ co2ice_lh(p) ! Mars
             ...
             if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
                ...
                if (ltype(l) /= isturb )then
                   ...
                   eflx_gnet_top = sabg_lyr(p,lyr_top) + dlrad(p) + (1-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) &
                        - lwrad_emit(c) - (eflx_sh_grnd(p)+qflx_evap_soi(p)*htvp(c)) !+ co2ice_lh(p) ! mars
       ...
       do fc = 1,num_nolakec
          ...
          if (j >= snl(c)+1) then
             if (j == snl(c)+1) then
                ...
                !  fn(c,j) = ithincol_i(c)*(t_soisno(c,j+1)-t_soisno(c,j))/(z(c,j+1)-z(c,j))  ! mars, not used?
       ...
       do fc = 1,num_nolakec
          ...
          if (j >= snl(c)+1) then
             if (j == snl(c)+1) then
                ...
                if (j <= 1) then
                   rt(c,j) = t_soisno(c,j) + cnfac*fact(c,j)*( fn(c,j) - fn(c,j-1) ) + (fact(c,j)*sabg_lyr_col(c,j)) !+ co2ice_lh(p) ! mars
                else
...
!
    real(r8), pointer :: ithincol(:)      !soil thermal conductivity from inertia  ! mars
    real(r8), pointer :: ithincol_i(:)    !soil thermal inertia factor for soil heating(I*sqrt(pi/P)) ! mars
...
!-----------------------------------------------------------------------
    ...
    ithincol   => cps%ithincol  ! mars, themal conductivty = I^2/(rho*cp)
    ithincol_i   => cps%ithincol_i ! mars, thermal inertia
          ...
          if (j >= 1) then    
             ...
             elseif (ltype(l) /= istwet .AND. ltype(l) /= istice  &
                ...
                if (satw > .1e-6_r8) then
                   ...
                   if ((ithincol(c) > 0.) ) then      ! adjust for use with thermal inertia (4/20/2011) ! mars
                      thk(c,j) = dke*dksat + (1.-dke)*ithincol(c)                  !mars 
                      thk(c,j) = ithincol(c)          ! since the thermal inertia field includes subsurface ice, ! mars
                  ...
                  if ((ithincol(c) > 0.) ) then   ! mars
                     thk(c,j) = ithincol(c)       ! mars
                   else                           ! mars 
