...
!----------------------------------------------------
  ...
  type, public :: atm2lnd_type
     ...
     real(r8), pointer :: forc_co2srf_snow(:)   => null() ! CO2 snow fall [kg/m2/s]  ! MARS_ETW
     real(r8), pointer :: forc_co2srf_cond(:)   => null() ! CO2 condensation [kg/m3/s]  ! MARS_ETW
...
!----------------------------------------------------
  ...
  type, public :: lnd2atm_type
     ...
     real(r8), pointer :: t_ref1m(:)      => null() !1m surface air temperature (Kelvin) ! mars
     ...
     real(r8), pointer :: co2dp(:)       => null()     ! CO2 frost depth ! mars
     real(r8), pointer :: co2_mass_change(:) => null() ! change in CO2 mass ! mars
     real(r8), pointer :: srf_emiss(:) => null()         ! surface emissivity ! mars   
     ...
     real(r8), pointer :: eflx_lh_co2(:)  => null() !co2 ice latent HF (W/m**2)  [+ to atm]  !Mars
...
!------------------------------------------------------------------------
  ...
  allocate(a2l%forc_co2srf_snow(beg:end))  ! MARS_ETW
  allocate(a2l%forc_co2srf_cond(beg:end))  ! MARS_ETW
  ...
  a2l%forc_co2srf_snow(beg:end) = ival   ! MARS_ETW
  a2l%forc_co2srf_cond(beg:end) = ival   ! MARS_ETW
...
!------------------------------------------------------------------------
  ...
  allocate(l2a%t_ref1m(beg:end))  ! mars
  allocate(l2a%q_ref2m(beg:end))
  allocate(l2a%u_ref10m(beg:end))
  allocate(l2a%h2osno(beg:end))
  allocate(l2a%co2dp(beg:end)) ! mars
  allocate(l2a%co2_mass_change(beg:end)) ! mars
  allocate(l2a%srf_emiss(beg:end)) ! mars    
  ...
  allocate(l2a%eflx_lh_co2(beg:end))
  ...
  l2a%t_ref1m(beg:end) = ival  ! mars
  l2a%q_ref2m(beg:end) = ival
  l2a%u_ref10m(beg:end) = ival
  l2a%h2osno(beg:end) = ival
  l2a%co2dp(beg:end) = ival ! mars
  l2a%co2_mass_change(beg:end) = ival ! mars
  l2a%srf_emiss(beg:end) = ival ! mars
...
!------------------------------------------------------------------------
  ...
  if (present(init)) then
     ...
     call c2g(begc, endc, begl, endl, begg, endg, cps%srf_emiss, clm_l2a%srf_emiss,&  ! mars  
          c2l_scale_type= 'urbanf', l2g_scale_type='unity')  ! Mars
     ...
     call c2g(begc, endc, begl, endl, begg, endg, cps%co2dp, clm_l2a%co2dp,&  ! mars
          c2l_scale_type= 'urbanf', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cps%srf_emiss, clm_l2a%srf_emiss,&  ! mars
          c2l_scale_type= 'urbanf', l2g_scale_type='unity')  ! Mars                        

    call c2g(begc, endc, begl, endl, begg, endg, cps%co2_mass_change, clm_l2a%co2_mass_change,&  ! mars
          c2l_scale_type= 'urbanf', l2g_scale_type='unity')
     ...
     call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &   ! mars
          pes%t_ref2m, clm_l2a%t_ref1m, &
          p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')
     ...
     call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &
          pef%eflx_lh_co2, clm_l2a%eflx_lh_co2, &
          p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')
