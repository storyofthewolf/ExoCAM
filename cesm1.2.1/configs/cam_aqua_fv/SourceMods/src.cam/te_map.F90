...
! !OUTPUT PARAMETERS:
   ...
   logical, intent(in)  ::  do_am_fixes  !switch for am correction + am fixer
...
! z
      ...
      !write(iulog,*) "uc:", uc, "vc:", vc, "du_s:", du_s, "du_w:", du_w
...
! ----------------------------------------------
          ...
          ! write(iulog,*) "ks:", grid%ks, "ak:", grid%ak, "bk:", grid%bk, "pe3:", pe3(1,:)
...
! ------------------------------
          call map1_ppm ( km,   pe0,    u,    km,   pe3,    u,            &
            ...
            if (do_am_fixes) then 
              ...
              !write(iulog,*) "max du_s:", maxval(du_s), "max du_w:", maxval(du_w), "max uc:", maxval(uc)
              do k=1,km
                  do i=i1,i2
                    du_w(i,j,k)=du_s(k)
                  enddo
