...
real(r8), parameter :: ncloud_co2 = 1.0e5            ! fixed cloud number concentration per kg of air
...
!   call pbuf_add_field('CO2_SURF_COND', 'physpkg', dtype_r8,(/pcols,pver/), cicewp_co2_idx)
  endif
...
!
  ...
  ! kludge
  call addfld('CLDICE_CO2_local', 'kg/kg   ', pver, 'A', 'CO2 ice cloud condensate local', phys_decomp )
  ...
  call addfld ('CO2_SRFICE_MASS','kg/m2',1,    'A','CO2 surface ice mass'   ,phys_decomp, sampling_seq=sampling_seq)
  call addfld ('CO2_SNOWFALL_RATE','kg/m2/s',1,    'A','CO2 snow fall rate'   ,phys_decomp, sampling_seq=sampling_seq)
  call addfld ('CO2_SRFICE_DEPTH','m',1,    'A','CO2 surface ice depth'   ,phys_decomp, sampling_seq=sampling_seq)
  call addfld ('CO2_SRFICE_FLUX','kg/m2/s',1,    'A','CO2 total surface ice flux'   ,phys_decomp, sampling_seq=sampling_seq)
...
!if (masterproc)  write(*,*), "after", ptend%q(i,k,ixcldice_co2)
  ...
  call outfld('CO2_SRFICE_DEPTH' ,ext_co2dp/SHR_CONST_DENSITYCO2FR    , pcols,lchnk)  ! remove?
  call outfld('CO2_SRFICE_MASS' ,ext_co2dp    , pcols,lchnk)                          ! remove?
  ...
  call outfld('CLDICE_CO2_local' ,cld_tmp    ,          pcols,lchnk)  ! remove??
...
!write(*,*) "V40, vslop", v40, vslope, v400, ext_reff(1,1)
  ! initialization
  cld_pvel_out(:,:) = 0.0
  ...
  !
  ! compute air density
  !rho(:ncol,:) = ext_pmid(:ncol,:) / (rair * ext_tmid(:ncol,:))  
