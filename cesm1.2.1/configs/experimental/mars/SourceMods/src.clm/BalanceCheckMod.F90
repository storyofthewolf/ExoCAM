...
!
    ...
    real(r8), pointer :: co2ice_lh(:)          ! co2 latent heat       ! mars
!
...
!
    ...
    real(r8), pointer :: errsoi_pft(:)      ! column-level soil/lake energy conservation error (W/m**2)  ! mars
       ...
       if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
          ...
          if (ltype(l) /= isturb) then
             errseb(p) = sabv(p) + sabg(p) + forc_lwrad(g) - eflx_lwrad_out(p) &
                         - eflx_sh_tot(p) - eflx_lh_tot(p) - eflx_soil_grnd(p) - co2ice_lh(p) !mars
          else
...
!!           write(iulog,*)'clm model warning, should be stopping?'
!!          write(iulog,*)'clm model is stopping'  ! mars
!!          call endrun()                          ! mars
!       end if
!    end if
...
! pft level
 found = .false.
  do p = lbp, ubp
    if (abs(errsoi_pft(p)) > 1.0e-7_r8 ) then
       found = .true.
       indexp = p
     end if
  end do
  if ( found  .and. (nstep > 2) ) then
!     write(iulog,100)'BalanceCheck: soil balance error, pft level',nstep,indexp,errsoi_pft(indexp)
   end if
