...
!!                        cappa, delpxy, ptxy, pexy, psxy, ptop)  !! C.Bardeen
                          cappa, delpxy, ptxy, dptdtxy, pexy, psxy, ptop) !! C.Bardeen
! !USES:
...
! !INPUT/OUTPUT PARAMETERS:
    ...
    real(r8), intent(inout) :: dptdtxy (grid%ifirstxy:grid%ilastxy,                      &     !! C.Bardeen
                                     grid%jfirstxy:grid%jlastxy, grid%km) ! Virtual pot temp tendency
...
! !LOCAL VARIABLES:
    ...
    !! C.Bardeen ntotq+3 (was +2)
    real(r8) :: pole(grid%ifirstxy:grid%ilastxy,grid%km,grid%ntotq+3)
                                               !  Array containing local pole values
    real(r8) :: pole_sum(grid%km,grid%ntotq+3) !  Array containing average of all pole values  !! C.Bardeen  
    real(r8) :: rel_diff(2,grid%km,grid%ntotq+3)
    ...
    if (jfirstxy==1) then
       ...
       do k = 1, km
          do i = ifirstxy, ilastxy
             ...
             pole(i,k,3) = dptdtxy(i,1,k)  !! C.Bardeen
          enddo
          do m = 1, ntotq
             do i = ifirstxy, ilastxy
!                pole(i,k,m+2) = tracer(i,1,k,m)  !! C. Bardeen
                pole(i,k,m+3) = tracer(i,1,k,m)  !! C. Bardeen
             enddo
          enddo
       enddo

       call t_startf("pdadj_reprosum")
       call shr_reprosum_calc(pole, pole_sum, lim, lim, km*(ntotq+3), gbl_count=im, &  !! C.Bardeen
       ...
       if ( shr_reprosum_tolExceeded('p_d_adjust/South Pole', km*(ntotq+3), &
       ...
       do k = 1, km
          ...
          do i = ifirstxy,ilastxy
             ...
             dptdtxy(i,1,k) = pole_sum(k,3)
          enddo
          do m = 1, ntotq
             do i = ifirstxy,ilastxy
                tracer(i,1,k,m) = pole_sum(k,m+3)
             enddo
          enddo
       enddo
    ...
    if (jlastxy==jm) then
       ...
       do k = 1, km
          do i = ifirstxy, ilastxy
             ...
             pole(i,k,3) = dptdtxy(i,jm,k)
          enddo
          do m = 1, ntotq
             do i = ifirstxy, ilastxy
                pole(i,k,m+3) = tracer(i,jm,k,m)
             enddo
          enddo
       enddo

       call t_startf("pdadj_reprosum")
       call shr_reprosum_calc(pole, pole_sum, lim, lim, km*(ntotq+3), gbl_count=im, &
       ...
       write_warning = .false.
       if (grid%myidxy_x == 0) write_warning = .true.
       if ( shr_reprosum_tolExceeded('p_d_adjust/Nouth Pole', km*(ntotq+3), &
                                   write_warning, iulog, rel_diff) ) then
          if ( shr_reprosum_recompute ) then
             call t_startf("pdadj_sumfix")
             allocate( pole_tmp(im) )
       ...
       do k = 1, km
          ...
          do i = ifirstxy,ilastxy
             ...
             dptdtxy(i,jm,k) = pole_sum(k,3)
          enddo
          do m = 1, ntotq
             do i = ifirstxy,ilastxy
                tracer(i,jm,k,m) = pole_sum(k,m+3)
             enddo
          enddo
       enddo
