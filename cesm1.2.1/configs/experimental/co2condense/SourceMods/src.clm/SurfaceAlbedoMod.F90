...
! !PUBLIC TYPES:
  ...
  use clm_varcon  , only : istcrop, albco2
  use shr_kind_mod, only : r8 => shr_kind_r8
  use clm_varpar  , only : nlevsno
  use SNICARMod   , only : sno_nbr_aer, SNICAR_RT, DO_SNO_AER, DO_SNO_OC
  use clm_varctl  , only : use_snicar_frc
  use SHR_CONST_mod , only : SHR_CONST_DENSITYCO2FR, SHR_CONST_CO2_ALVIS, &   ! mars
                             SHR_CONST_CO2_ALNIR, SHR_CONST_CO2_ALS, SHR_CONST_CO2_ALN ! mars
  use exoplanet_mod   , only : exo_lnd_albglac_vis, exo_lnd_albglac_ifr
...
! This is the value used in CAM3 by Pritchard et al., GRL, 35, 2008.
 ...
 !                      (/ 0.80_r8, 0.55_r8 /) 
                        (/ exo_lnd_albglac_vis, exo_lnd_albglac_ifr /)    ! mars
!
...
! !ARGUMENTS:
    ...
    real(r8), intent(in) :: calday                     ! not used
...
!
    ...
    real(r8), pointer :: frac_co2(:)     ! fraction of ground covered by CO2 ! mars
...
!
  ...
  !-----------------------------------------------------------------------
   ...
   frac_co2   => cps%frac_co2  ! mars
    ...
    do g = lbg, ubg
       coszen_gcell(g) = shr_orb_cosz (frac_day, lat(g), lon(g), declinp1)
    end do
    ...
    do ib = 1, nband
       do fc = 1,num_nourbanc
          ...
          if (coszen(c) > 0._r8) then
             ...
             ! if the co2 frost fraction is greater than snow fraction, use co2 frost albedo
             
!write(*,*) "frac_co2", frac_co2(c)
             if (frac_co2(c) > frac_sno(c)) then      ! mars
!                albgrd(c,ib) = albsod(c,ib)*(1.-frac_co2(c)) + albsnd(c,ib)*frac_co2(c)  ! mars
!                albgri(c,ib) = albsoi(c,ib)*(1.-frac_co2(c)) + albsni(c,ib)*frac_co2(c)  ! mars
                albgrd(c,ib) = albsod(c,ib)*(1.-frac_co2(c)) + albco2(ib)*frac_co2(c)  ! mars
                albgri(c,ib) = albsoi(c,ib)*(1.-frac_co2(c)) + albco2(ib)*frac_co2(c)  ! mars
!write(*,*) "co2 albedos", albgrd(c,ib), albgri(c,ib)
             else   ! snow fraction greater than co2 frost, use snow albedo defined by SNICAR
       ...
       do fc = 1,num_nourbanc
          ...
          if (coszen(c) > 0._r8) then
             ...
             if (ltype(l) == istsoil .or. ltype(l) == istcrop)  then ! soil
               ...
               if (co2dp(c) > 0.05_r8) then  ! mars  IF glacier covered in CO2 ice
                 albsod(c,ib) = albco2(ib)   ! mars
                 albsoi(c,ib) = albco2(ib)   ! mars
               else                          ! else glacier is bare of CO2 ice
                 ! changed from local variable to clm_type:
                 !albsod = albice(ib)
                 !albsoi = albsod
               ...
               if (co2dp(c) > 0.05_r8) then  ! mars  IF frozen ground covered in CO2 ice
                 albsod(c,ib) = albco2(ib)   ! mars
                 albsoi(c,ib) = albco2(ib)   ! mars
               else      
