...
!
    ...
    type (parpatterntype)   :: u_to_uxy, uxy_to_u, v_to_vxy, vxy_to_v,             &
                               dudt_to_dudtxy, dudtxy_to_dudt,                     &
                               dvdt_to_dvdtxy, dvdtxy_to_dvdt,                     &
                               dptdt_to_dptdtxy, dptdtxy_to_dptdt,                 &
...
!
   !!!     call t_startf('transpose_creation')
       ...
       if (gid .lt. npes_xy) then
        ...
        call parpatterncreate(commxy, grid%ghostu_yz, grid%strip3kxyz, &
                              grid%dudt_to_dudtxy, mod_method=grid%mod_method)
        call parpatterncreate(commxy, grid%strip3kxyz,grid%ghostu_yz, &
                              grid%dudtxy_to_dudt, mod_method=grid%mod_method)
        ...
        call parpatterncreate(commxy, grid%ghostv_yz, grid%strip3kxyz, &
                              grid%dvdt_to_dvdtxy, mod_method=grid%mod_method)
        call parpatterncreate(commxy, grid%strip3kxyz, grid%ghostv_yz, &
                              grid%dvdtxy_to_dvdt, mod_method=grid%mod_method)
...
!
        ...
        call parpatterncreate(commxy, grid%ghostpt_yz, grid%strip3kxyz,  &
                              grid%dptdt_to_dptdtxy, mod_method=grid%mod_method)
        call parpatterncreate(commxy, grid%strip3kxyz, grid%ghostpt_yz,  &
                              grid%dptdtxy_to_dptdt, mod_method=grid%mod_method)
        call parpatterncreate(commxy, grid%strip3dxyz, grid%strip3kxyz,  &
...
!
        ...
        call parpatternfree(grid%commxy, grid%dudt_to_dudtxy)
        call parpatternfree(grid%commxy, grid%dudtxy_to_dudt)
        call parpatternfree(grid%commxy, grid%v_to_vxy)
        call parpatternfree(grid%commxy, grid%vxy_to_v)
        call parpatternfree(grid%commxy, grid%dvdt_to_dvdtxy)
        call parpatternfree(grid%commxy, grid%dvdtxy_to_dvdt)
        ...
        call parpatternfree(grid%commxy, grid%dptdt_to_dptdtxy)
        call parpatternfree(grid%commxy, grid%dptdtxy_to_dptdt)
