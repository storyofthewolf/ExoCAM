...
!-------------------------------------------------------------------------------
   ...
   real   (SHR_KIND_R8),parameter :: ve     = SHR_CONST_CDVE   ! Calday of vernal equinox
   ...
   real   (SHR_KIND_R8) ::   datesec ! Date in seconds since vernal equinox
   real   (SHR_KIND_R8) ::   Ls      ! Areocentric longitude of the planet (radians)
...
!   invrho = (1._SHR_KIND_R8 + eccen*cos(lamb - mvelpp)) / (1._SHR_KIND_R8 - eccen*eccen)  ! notMars
 
   ! Set solar declination and eccentricity factor
 
!   delta  = asin(sin(obliqr)*sin(lamb))  ! notMars
!   eccf   = invrho*invrho                ! notMars
 
   ! Mars
   datesec = calday*SHR_CONST_SECDAYD
   Ls = shr_orb_findLs(datesec,eccen)
   delta = asin(sin(obliqr)*sin(Ls))
   invrho = (1. + eccen*cos(Ls - mvelpp)) / (1. - eccen*eccen)
   eccf   = invrho*invrho
...
! ================================ 
!--------------------------------------------------------------------------------
!  Find the Eccentric anomaly of the orbit.                                                                                              
!  by Kaj Williams    1/10/2007                                                                                                          
!                                                                                                                                        
!   inputs:   t  -  Current time in seconds. Time is measured from vernal equinox of Mars.
!             eccen  -   orbital eccentricity 
!   output:   eccentric anomaly in radians. 
!
!-----------------------------------------------------------------------------------

real(SHR_KIND_R8) FUNCTION anome(t,eccen)

  ! Input arguments
  real(SHR_KIND_R8),intent(in)  ::  t     
  real(SHR_KIND_R8),intent(in)  ::  eccen

  !---------------------Local Variables----------------------                                                                      
  real(SHR_KIND_R8) :: n       = 0.0      ! n is the mean motion (in rad/sec)                                                        
  real(SHR_KIND_R8) :: deltae  = 0.0      ! parameters for Newton's method:                                                          
  real(SHR_KIND_R8) :: tol     = 1.0d-11  ! tol is the acceptable error for Newton's method                                          
  real(SHR_KIND_R8) :: m = 0.0, E = 0.0, y = 0.0, y_prime = 0.0, E_new = 0.0, true_anomaly = 0.0, Ls = 0.0 
  real(SHR_KIND_R8) :: solar_year_in_seconds=SHR_CONST_SECYEAR

  !---------------------Execution----------------------------                                                              
  n = 2.0 * Pi / solar_year_in_seconds

  ! first compute the mean anomaly m:                                                                                      
  m = n * (t+(solar_year_in_seconds - SHR_CONST_TPERI) )

  ! next compute eccentric anomaly E                                                                                       
  ! we need to solve m=E - ecc * sin(E)                                                                                    
  ! with the Newton method...                                                                                              
  ! as first guess let E=m                                                                                                 
  E = m

  do
    y = m - E + eccen * sin(E)
    y_prime = -1 + eccen * cos(E)
    E_new = E - y / y_prime
    deltae = E_new - E
    E = E_new
    if (abs(deltae) < tol) exit
  enddo
...
!-------------------------------------------------------------------------------------
!  Find the areocentric longitude given time (in seconds) since vernal equinox.
!  by Kaj Williams    1/10/2007
!
!   inputs:   t  -  Current time in seconds. Time is measured from vernal equinox for Mars
!             eccen  -   orbital eccentricity
!   output:   Areocentric longitude in radians.
!
!--------------------------------------------------------------------------------------

real(SHR_KIND_R8) FUNCTION shr_orb_findLs(t,eccen)
  ...
  ! ------------------local variables-------------- 
  real(SHR_KIND_R8):: E = 0.0, true_anomaly = 0.0, Ls = 0.0 

  E=anome(t,eccen)

  if (abs(E) < Pi) then
    true_anomaly = 2 * atan(sqrt((1 + eccen) / (1 - eccen)) * tan(E / 2.0))
  else
    true_anomaly = 2 * atan(sqrt((1 + eccen) / (1 - eccen)) * tan(E / 2.0)) + (2.0 * Pi)
  endif

  Ls = true_anomaly - SHR_CONST_LONVE

  shr_orb_findLs=Ls

end function shr_orb_findLS
