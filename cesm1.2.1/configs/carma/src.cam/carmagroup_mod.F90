...
contains
  ...
  subroutine CARMAGROUP_Create(carma, igroup, name, rmin, rmrat, ishape, eshape, is_ice, rc, is_fractal, &
    ...
    integer, optional, intent(in)               :: imiertn             !! mie routine [I_MIERTN_TOON1981 | I_MIERTN_BOHREN1983 | I_MIERTN_FRACTAL]
    ...
    !!
    !! If the user desires to use fractal optical properties calculation (I_MIERTN_FRACTAL), then
    ...
    if ((carma%f_group(igroup)%f_imiertn .eq. I_MIERTN_FRACTAL) .and. (carma%f_group(igroup)%f_is_fractal .eq. .FALSE.)) then        
    ...
    if (carma%f_group(igroup)%f_is_fractal .eq. .TRUE. .or. carma%f_group(igroup)%f_imiertn .eq. I_MIERTN_FRACTAL) then
  ...
  subroutine CARMAGROUP_Get(carma, igroup, rc, name, shortname, rmin, rmrat, ishape, eshape, is_ice, is_fractal, &
    ...
    real(kind=f), intent(out), optional       :: rprat(carma%f_NBIN)             !! the porusity radius ratio (scaled porosity radius / equiv. sphere)
    ...
    integer, optional, intent(out)            :: imiertn                      !! mie routine [I_MIERTN_TOON1981 | I_MIERTN_BOHREN1983 | I_MIERTN_FRACTAL]
