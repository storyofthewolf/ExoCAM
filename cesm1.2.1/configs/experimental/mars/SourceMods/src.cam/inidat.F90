...
!
    ...
    use exo_condense_mod, only: exo_condense_implements_cnst, exo_condense_init_cnst
!
! Input arguments
!
    type(file_desc_t),           intent(inout)    :: ncid_ini
...
if(masterproc) write(*,*) "inidat", (exo_condense_implements_cnst(cnst_name(m_cnst)))
          else if (exo_condense_implements_cnst(cnst_name(m_cnst))) then
if(masterproc) write(*,*) cnst_name(m_cnst)
             call exo_condense_init_cnst(cnst_name(m_cnst),q3tmp , gcid)
             if(masterproc) write(iulog,*) '          ', cnst_name(m_cnst), ' initialized by "exo_condense_init_cnst"'
          else
