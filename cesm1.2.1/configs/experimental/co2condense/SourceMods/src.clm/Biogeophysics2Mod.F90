...
!-----------------------------------------------------------------------
    ...
    do fp = 1,num_nolakep
       ...
       if (egirat(c) < 1.0_r8) then
          ...
          eflx_sh_grnd(p) = eflx_sh_grnd(p) + (save_qflx_evap_soi - qflx_evap_soi(p))*htvp(c)  ! original
      ...
      if (abs(eflx_sh_grnd(p)) > 1000.) then
       ...
       if (ltype(l) /= isturb) then
          eflx_soil_grnd(p) = sabg(p) + dlrad(p) &
                              ...
                              - (eflx_sh_grnd(p) + qflx_evap_soi(p)*htvp(c)) !+ co2ice_lh(p)

! this should include CO2 latent heat?  Latent heat from CO2 is already included via temperature adjustment in condensation routine
...
!                              "eflx_sh_grnd", eflx_sh_grnd(p), "qflx_evap_soi*htvp", qflx_evap_soi(p)*htvp(c), "co2ice_lh", co2ice_lh(p)
       ...
       if ((abs(qflx_evap_tot(p)) < 1.e-14_r8) .or.(isnan(qflx_evap_tot(p)))) then  ! mars
         !write(6,*) 'qflx_tot is:',qflx_evap_tot(p), p  ! mars
         qflx_evap_tot(p) = 0._r8  ! mars
         qflx_evap_veg(p) = 0._r8  ! mars
         qflx_evap_soi(p) = 0._r8  ! mars
       endif ! mars
       ! this gets passed to atmosphere model
...
!write(*,*)  "p, errsoi_pft(p)", "p=",p, "errsoi_pft=",errsoi_pft(p), "eflx_soil_grnd=",eflx_soil_grnd(p), "xmf=",xmf(c), "co2ice_lh=",co2ice_lh(p) ! mars   
...
!          if(co2ice_lh(p) .ne. 0.0) then
!write(*,*) "-------------------------"
!write(*,*)    "p, errsoi_pft 2", "p=",p, "j=",j, "errsoi_pft(p)=",errsoi_pft(p), "tsoisno-tssbet/fact=", (t_soisno(c,j)-tssbef(c,j))/fact(c,j), "tdif=",t_soisno(c,j)-tssbef(c,j), "fact=",fact(c,j)
!          end if           
          end if
       end do
    end do
