...
! !USES:
    ...
    use shr_const_mod      , only : SHR_CONST_RGAS, SHR_CONST_CPDAIR, SHR_CONST_KVISC, &
                                    SHR_CONST_CO2ICE_THICK, SHR_CONST_DENSITYCO2FR
...
!if(masterproc) write(*,*) "before MoninObukIni", z0mg_pft(p), z0mg_col(c), z0hg_pft(p), z0hg_col(c), z0qg_pft(p), z0qg_col(c)
!if(masterproc) write(*,*) "MoninObukIni", ur(p), thv(c), dthv, zldis(p), z0mg_pft(p), um(p), obu(p)
...
!       if(masterproc) write(*,*) "bareground, rah", rah, temp1(p), ustar(p)
!       if(masterproc) write(*,*) "bareground, raih", raih, forc_rho(c), cpair, rah
