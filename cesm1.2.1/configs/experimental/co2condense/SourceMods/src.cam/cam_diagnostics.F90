...
use shr_const_mod, only: SHR_CONST_DENSITYCO2FR
...
subroutine diag_init()
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   call addfld ('LS      ','degrees ',1,    'A','Areocentric Longitude',phys_decomp)
   ...
   if(do_exo_condense_co2) then
     call addfld ('CO2DP','kg/m2           ',1,    'A','Carbon dioxide frost mass',phys_decomp, fill_value = 1.e30_r8)  
     call addfld ('CO2MASSCHANGE','kg/m2   ',1,    'A','Carbon dioxide frost mass change',phys_decomp, fill_value = 1.e30_r8) 
     call addfld ('CO2_LHFLX','W/m2        ',1,    'A','Carbon dioxide frost latent heat flux',phys_decomp, fill_value = 1.e30_r8) 
     call addfld ('CO2DPz','m',1,    'A','CO2 surface ice depth'   ,phys_decomp, fill_value = 1.0e30_r8)                   
   endif

   call addfld ('TBOT    ','K       ',1,    'A','Lowest model level temperature', phys_decomp)

   call addfld ('SRF_EMISS',   '1',       1,    'A','surface emissivity', phys_decomp)
...
!-----------------------------------------------------------------------
    ...
    use time_manager,       only: get_nstep, get_curr_calday_rotation ! mars
    use interpolate_data,   only: vertinterp
    use constituent_burden, only: constituent_burden_comp
    use cam_control_mod,    only: moist_physics, eccen ! mars
    use co2_cycle,          only: c_i, co2_transport

    use tidal_diag,         only: tidal_diag_write
    use shr_orb_mod,        only: shr_orb_findLs  ! mars
    use shr_const_mod,      only: SHR_CONST_SECYEAR, SHR_CONST_PI ! mars
!-----------------------------------------------------------------------
!
! Arguments
!
...
!
    ...
    real(r8) outLs(pcols)       !used for outfld call  ! Mars
    real(r8) frac_day, frac_year, time ! Mars
!-----------------------------------------------------------------------
!
    lchnk = state%lchnk
    ncol  = state%ncol
   ...
   !if (mon < 12) then
   !   days = real(56*(mon-1) + day)
   !else
   !   days = real(616 + day)    ! 616 is 56*11
   !endif
   !time = days*shr_const_secdayr + ncsec
   !outLs(:ncol) = 360.0*shr_orb_findLs(time,eccen)/shr_const_twopi

   call get_curr_calday_rotation(frac_day, frac_year)  ! mars
   time = frac_year*SHR_CONST_SECYEAR                  ! mars
   outLS(:ncol) = 360.0*shr_orb_findLs(time,eccen)/(2.0*SHR_CONST_PI) ! mars
   call outfld('LS',outLs, pcols, lchnk)  ! mars
...
!
    ...
    call outfld('SRF_EMISS',      cam_in%srf_emiss,        pcols, lchnk)
    if(do_exo_condense_co2) then 
      call outfld('CO2DP',          cam_in%co2dp,            pcols, lchnk)
      call outfld('CO2MASSCHANGE',  cam_in%co2_mass_change,  pcols, lchnk)
      call outfld('CO2_LHFLX',      cam_in%co2_lhflx,        pcols, lchnk)
      call outfld('CO2DPz' ,cam_in%co2dp/SHR_CONST_DENSITYCO2FR    , pcols,lchnk) 
    endif
    if (co2_transport()) then
       do m = 1,4
