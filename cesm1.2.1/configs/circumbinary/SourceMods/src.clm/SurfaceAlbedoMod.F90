...
!Wolf          endif
    ...
    do ib = 1, nband
       do fc = 1,num_nourbanc
          c = filter_nourbanc(fc)
!Wolf          if ((coszen(c) > 0._r8) .and. (h2osno(c) > 0._r8)) then
             albsnd_hst(c,ib) = albsnd(c,ib)
             albsni_hst(c,ib) = albsni(c,ib)
!Wolf          else
!Wolf             albsnd_hst(c,ib) = 0._r8
!Wolf             albsni_hst(c,ib) = 0._r8
!Wolf          endif
       enddo
    enddo
    ...
    do fp = 1,num_nourbanp
       p = filter_nourbanp(fp)
!Wolf          if (coszen_pft(p) > 0._r8) then
